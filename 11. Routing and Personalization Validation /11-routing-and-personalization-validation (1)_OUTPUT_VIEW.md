...
ART_DIR = 'artifacts_routing_personalization_validation_kaggle_fixed'
...
def build_dense_view(flat_view, n_global_ids):
    out = []
    for v in flat_view:
        ...
        clients = [(Xtr[idx], ytr[idx]) for idx in v['cidx']]
        out.append({'name': v['meta']['name'], 'tr': (Xtr, ytr), 'va': (Xva, yva), 'te': (Xte, yte), 'clients': clients})
    return out

class DenseTensorDataset(torch.utils.data.Dataset):
...
def merge_state(base_state, update_state):
    out = {k: v.clone() for k, v in base_state.items()}
    for k, v in update_state.items():
        out[k] = v.clone()
    return out

class RouteFusionModel(nn.Module):
    def __init__(self, in_shared, in_private, d_shared=128, d_private=64, n_classes=2):
        super().__init__()
        self.in_shared = int(in_shared)
        self.in_private = int(in_private)
        if self.in_shared > 0:
            self.shared_encoder = nn.Sequential(
                nn.Linear(self.in_shared, d_shared),
                nn.GELU(),
                nn.Dropout(0.10),
            ...
            shared_dim = 0
        if self.in_private > 0:
            self.private_encoder = nn.Sequential(
                nn.Linear(self.in_private, d_private),
                nn.GELU(),
                nn.Dropout(0.10),
                nn.Linear(d_private, d_private),
                nn.GELU(),
                nn.Dropout(0.10),
            )
            private_dim = d_private
        else:
            self.private_encoder = None
            private_dim = 0
        fused_dim = max(1, shared_dim + private_dim)
        self.head = nn.Linear(fused_dim, n_classes)
    def forward(self, xs, xp):
        feats = []
        if self.shared_encoder is not None:
            feats.append(self.shared_encoder(xs))
        if self.private_encoder is not None:
            feats.append(self.private_encoder(xp))
        if not feats:
            z = torch.zeros((xs.size(0), 1), device=xs.device)
        elif len(feats) == 1:
            z = feats[0]
        else:
...
class SingleTokenizerTransformerModel(nn.Module):
    def __init__(self, in_dim, d_model=96, n_heads=4, n_layers=2, ff_dim=192, n_classes=2):
        super().__init__()
        self.in_dim = int(in_dim)
        self.scalar_w = nn.Parameter(torch.randn(self.in_dim, d_model) * 0.02)
        self.scalar_b = nn.Parameter(torch.zeros(self.in_dim, d_model))
        self.feature_emb = nn.Parameter(torch.randn(self.in_dim, d_model) * 0.02)
    ...
    def forward(self, x):
        b = x.size(0)
        toks = x.unsqueeze(-1) * self.scalar_w.unsqueeze(0) + self.scalar_b.unsqueeze(0) + self.feature_emb.unsqueeze(0)
...
class LayerOnlyPersonalizationModel(nn.Module):
...
def train_route_model(model, Xs, Xp, y, epochs=1, lr=1e-3):
    ...
    for _ in range(epochs):
        for xs, xp, yb in loader:
            ...
            opt.zero_grad(set_to_none=True)
            loss = F.cross_entropy(model(xs, xp), yb)
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            opt.step()

def train_single_input_model(model, X, y, epochs=1, lr=1e-3):
    ds = DenseTensorDataset(X, y)
    loader = DataLoader(ds, batch_size=min(BATCH_SIZE, max(128, len(ds))), shuffle=True, num_workers=NUM_WORKERS, pin_memory=PIN_MEM, drop_last=False)
    opt = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1e-4)
    model.train()
    for _ in range(epochs):
...
def predict_route_model(model, Xs, Xp, batch_size=2048):
    model.eval()
    out = []
    with torch.no_grad():
        for i in range(0, len(Xs), batch_size):
            xs = torch.tensor(Xs[i:i+batch_size], dtype=torch.float32, device=DEVICE)
            xp = torch.tensor(Xp[i:i+batch_size], dtype=torch.float32, device=DEVICE)
            out.append(torch.softmax(model(xs, xp), dim=1).cpu().numpy())
    return np.concatenate(out, axis=0)

def predict_single_input_model(model, X, batch_size=2048):
    model.eval()
    out = []
    with torch.no_grad():
...
def evaluate_models_dense(view, models, predict_fn):
    val_metrics_each, test_metrics_each = [], []
    ...
    for v, model in zip(view, models):
        ...
        pva = predict_fn(model, Xva_s, 'va')
        pte = predict_fn(model, Xte_s, 'te')
...
def run_no_routing_setting(name, dense_view, n_global_ids):
    models = []
    for i, _ in enumerate(dense_view):
        torch.manual_seed(SEED + i)
        models.append(RouteFusionModel(n_global_ids, n_global_ids).to(DEVICE))
    global_shared = state_to_cpu(models[0].shared_encoder)
    for _ in range(2):
        local_shared, weights = [], []
        for i, v in enumerate(dense_view):
            m = models[i]
            load_state_safe(m.shared_encoder, global_shared)
            for Xc, yc in v['clients']:
                train_route_model(m, Xc, Xc, yc, epochs=1, lr=LR)
            local_shared.append(state_to_cpu(m.shared_encoder))
            weights.append(max(1, sum(len(yc) for _, yc in v['clients'])))
        weights = np.array(weights, np.float32); weights = weights / weights.sum()
        global_shared = average_states(local_shared, weights)
    for m in models:
        load_state_safe(m.shared_encoder, global_shared)
    def pred_fn(model, X, split_name):
        return predict_route_model(model, X, X)
    res = evaluate_models_dense(dense_view, models, pred_fn)
    res['method'] = name
    return res

def run_single_tokenizer_setting(name, dense_view, n_global_ids):
    models = []
    for i, _ in enumerate(dense_view):
        torch.manual_seed(SEED + i)
        models.append(SingleTokenizerTransformerModel(n_global_ids).to(DEVICE))
    global_nonhead = {k: v for k, v in state_to_cpu(models[0]).items() if not k.startswith('head.')}
    for _ in range(2):
        local_nonhead, weights = [], []
        for i, v in enumerate(dense_view):
            m = models[i]
            base = state_to_cpu(m)
            load_state_safe(m, merge_state(base, global_nonhead))
            for Xc, yc in v['clients']:
                train_single_input_model(m, Xc, yc, epochs=1, lr=LR)
            local_nonhead.append({k: v for k, v in state_to_cpu(m).items() if not k.startswith('head.')})
            weights.append(max(1, sum(len(yc) for _, yc in v['clients'])))
        weights = np.array(weights, np.float32); weights = weights / weights.sum()
        global_nonhead = average_states(local_nonhead, weights)
    for m in models:
        base = state_to_cpu(m)
        load_state_safe(m, merge_state(base, global_nonhead))
    def pred_fn(model, X, split_name):
        return predict_single_input_model(model, X)
    res = evaluate_models_dense(dense_view, models, pred_fn)
    res['method'] = name
    return res

def run_layer_only_personalization(name, dense_view, n_global_ids):
    models = []
    for i, _ in enumerate(dense_view):
        torch.manual_seed(SEED + i)
        models.append(LayerOnlyPersonalizationModel(n_global_ids).to(DEVICE))
    ...
    for _ in range(2):
        ...
        for i, v in enumerate(dense_view):
            ...
            weights.append(max(1, sum(len(yc) for _, yc in v['clients'])))
        weights = np.array(weights, np.float32); weights = weights / weights.sum()
        global_backbone = average_states(local_backbones, weights)
    for m in models:
        load_state_safe(m.backbone, global_backbone)
    def pred_fn(model, X, split_name):
        return predict_single_input_model(model, X)
...
def run_all_shared_setting(name, dense_view, n_global_ids):
    models = []
    for i, _ in enumerate(dense_view):
        torch.manual_seed(SEED + i)
        models.append(RouteFusionModel(n_global_ids, 0).to(DEVICE))
    global_shared = state_to_cpu(models[0].shared_encoder)
    for _ in range(2):
        local_shared, weights = [], []
        for i, v in enumerate(dense_view):
            ...
            load_state_safe(m.shared_encoder, global_shared)
            for Xc, yc in v['clients']:
                train_route_model(m, Xc, np.zeros((len(Xc), 0), np.float32), yc, epochs=1, lr=LR)
            local_shared.append(state_to_cpu(m.shared_encoder))
            weights.append(max(1, sum(len(yc) for _, yc in v['clients'])))
        weights = np.array(weights, np.float32); weights = weights / weights.sum()
        global_shared = average_states(local_shared, weights)
    for m in models:
        load_state_safe(m.shared_encoder, global_shared)
    def pred_fn(model, X, split_name):
        return predict_route_model(model, X, np.zeros((len(X), 0), np.float32))
    res = evaluate_models_dense(dense_view, models, pred_fn)
    res['method'] = name
    return res

def run_all_private_setting(name, dense_view, n_global_ids):
    models = []
    for i, _ in enumerate(dense_view):
        torch.manual_seed(SEED + i)
        models.append(RouteFusionModel(0, n_global_ids).to(DEVICE))
    for _ in range(2):
        for i, v in enumerate(dense_view):
            m = models[i]
            for Xc, yc in v['clients']:
                train_route_model(m, np.zeros((len(Xc), 0), np.float32), Xc, yc, epochs=1, lr=LR)
    def pred_fn(model, X, split_name):
        return predict_route_model(model, np.zeros((len(X), 0), np.float32), X)
    res = evaluate_models_dense(dense_view, models, pred_fn)
    res['method'] = name
    return res
...
dense_view = build_dense_view(selected_flat_view, n_global_flat_ids)

results = []
results.append(run_no_routing_setting('No_Routing_Setting', dense_view, n_global_flat_ids))
results.append(run_single_tokenizer_setting('Single_Tokenizer_Setting', dense_view, n_global_flat_ids))
results.append(run_layer_only_personalization('Layer_Only_Personalization', dense_view, n_global_flat_ids))
results.append(run_all_shared_setting('All_Shared_Routing', dense_view, n_global_flat_ids))
results.append(run_all_private_setting('All_Private_Routing', dense_view, n_global_flat_ids))
...
show_table(
    'ROUTING AND PERSONALIZATION VALIDATION — GLOBAL METRICS',
    report,
    index=False,
    columns=cols,
...
plot_overlay(results, 'val', 'roc', os.path.join(ART_DIR, 'plots', 'routing_personalization_val_roc.png'), 'Routing and Personalization Validation — Validation ROC-AUC')
plot_overlay(results, 'test', 'roc', os.path.join(ART_DIR, 'plots', 'routing_personalization_test_roc.png'), 'Routing and Personalization Validation — Test ROC-AUC')
plot_overlay(results, 'val', 'pr', os.path.join(ART_DIR, 'plots', 'routing_personalization_val_pr.png'), 'Routing and Personalization Validation — Validation PR-AUC')
plot_overlay(results, 'test', 'pr', os.path.join(ART_DIR, 'plots', 'routing_personalization_test_pr.png'), 'Routing and Personalization Validation — Test PR-AUC')
...
with open(os.path.join(ART_DIR, 'routing_personalization_validation_metrics.json'), 'w') as f:
    json.dump(metrics_json, f, indent=2)
with open(os.path.join(ART_DIR, 'routing_personalization_validation_roc_pr_curves.json'), 'w') as f:
    json.dump(curves_json, f, indent=2)

print(os.path.join(ART_DIR, 'routing_personalization_validation_metrics.json'))
print(os.path.join(ART_DIR, 'routing_personalization_validation_roc_pr_curves.json'))
```
...
<div class="grip-table-block"><div class="grip-table-title">ROUTING AND PERSONALIZATION VALIDATION — GLOBAL METRICS</div><table class="dataframe">
  <thead>
  ...
  <tbody>
    ...
    <tr>
      <td>VAL</td>
      <td>Single_Tokenizer_Setting</td>
      <td>0.1672</td>
      <td>0.0836</td>
      <td>0.5000</td>
      <td>0.1276</td>
    ...
    <tr>
      <td>VAL</td>
      <td>Layer_Only_Personalization</td>
      <td>0.8369</td>
      <td>0.8524</td>
      <td>0.5173</td>
      <td>0.4875</td>
...
![png](11-routing-and-personalization-validation%20%281%29_OUTPUT_VIEW_files/11-routing-and-personalization-validation%20%281%29_OUTPUT_VIEW_0_2.png)
    



    
![png](11-routing-and-personalization-validation%20%281%29_OUTPUT_VIEW_files/11-routing-and-personalization-validation%20%281%29_OUTPUT_VIEW_0_3.png)
    



    
![png](11-routing-and-personalization-validation%20%281%29_OUTPUT_VIEW_files/11-routing-and-personalization-validation%20%281%29_OUTPUT_VIEW_0_4.png)
    



    
![png](11-routing-and-personalization-validation%20%281%29_OUTPUT_VIEW_files/11-routing-and-personalization-validation%20%281%29_OUTPUT_VIEW_0_5.png)
    


    artifacts_routing_personalization_validation_kaggle_fixed/routing_personalization_validation_metrics.json
    artifacts_routing_personalization_validation_kaggle_fixed/routing_personalization_validation_roc_pr_curves.json
