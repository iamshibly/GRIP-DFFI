...
LR = 1e-3
BATCH_SIZE = 256
SPLIT_RATIO_ARTIFACT_DIR = "artifacts_split_ratio"
SPLIT_RATIO_CONFIGS = [
    ("60_20_20", 0.60, 0.20, 0.20),
    ("80_10_10", 0.80, 0.10, 0.10),
]
CURRENT_SPLIT_RATIO = (0.70, 0.15, 0.15)
os.makedirs(SPLIT_RATIO_ARTIFACT_DIR, exist_ok=True)
...
def split_three_way(df, y_raw, seed=SEED):
    ...
    df = df.loc[mask].reset_index(drop=True)
    y_raw = pd.Series(y_raw).loc[mask].reset_index(drop=True)
    if total_eval_ratio <= 0 or train_ratio <= 0:
...
def new_model_for_ratio(meta, shared_feature_count):
...
def train_federated_model_for_ratio(meta_list, train_sets, val_sets, client_indices, shared_feature_count):
    seed_models = [new_model_for_ratio(meta, shared_feature_count) for meta in meta_list]
    ...
    for _ in range(1, ROUNDS + 1):
        ...
        for di, (meta, tr_ds, cidx) in enumerate(zip(meta_list, train_sets, client_indices)):
            for idx in cidx:
                ...
                model = new_model_for_ratio(meta, shared_feature_count)
...
def evaluate_ratio(split_ratio_name, meta_list, val_sets, test_sets, best_bundle, shared_feature_count):
    rows = []
    val_metrics_each = []
    test_metrics_each = []
    val_ns = []
    ...
    for di, meta in enumerate(meta_list):
        ...
        rows.append({"split_ratio": split_ratio_name, "split": "VAL", "dataset": meta["name"], **mv})
        rows.append({"split_ratio": split_ratio_name, "split": "TEST", "dataset": meta["name"], **mt})
...
def render_final_table(df):
    ...
    column_order = [
        "split_ratio", "split", "dataset", "acc", "prec_macro", "rec_macro", "f1_macro",
    ...
    for c in view.columns:
        if c not in ["split_ratio", "split", "dataset"]:
            view[c] = pd.to_numeric(view[c], errors="coerce").map(lambda x: "" if pd.isna(x) else f"{x:.4f}")
    view.to_csv(os.path.join(SPLIT_RATIO_ARTIFACT_DIR, "split_ratio_robustness_final_report.csv"), index=False)
    if IN_NOTEBOOK and HTML is not None:
        ...
        html += '<div style="padding:10px 14px;font-weight:700;color:#0f172a;background:#f8fafc;border-bottom:1px solid #e2e8f0;font-size:15px;">FINAL REPORT - SPLIT-RATIO ROBUSTNESS VALIDATION</div>'
        html += table_html + '</div>'
        display(HTML(html))
    else:
...
def run_one_split_ratio(split_ratio_name, train_ratio, val_ratio, test_ratio, seed_offset):
    global CURRENT_SPLIT_RATIO
    CURRENT_SPLIT_RATIO = (train_ratio, val_ratio, test_ratio)
    ...
    best_bundle = train_federated_model_for_ratio(all_meta, all_tr, all_va, all_cidx, shared_feature_count)
    rows = evaluate_ratio(split_ratio_name, all_meta, all_va, all_te, best_bundle, shared_feature_count)
...
def run_split_ratio_robustness_validation():
    all_rows = []
    for seed_offset, (split_ratio_name, train_ratio, val_ratio, test_ratio) in enumerate(SPLIT_RATIO_CONFIGS):
        rows = run_one_split_ratio(split_ratio_name, train_ratio, val_ratio, test_ratio, seed_offset)
        all_rows.extend(rows)
    report = pd.DataFrame(all_rows)
    report.to_csv(os.path.join(SPLIT_RATIO_ARTIFACT_DIR, "split_ratio_robustness_final_report_raw.csv"), index=False)
    render_final_table(report)
    return report


if __name__ == "__main__":
    run_split_ratio_robustness_validation()
```


<div style="margin:12px 0 18px 0;border:1px solid #dbe4ee;border-radius:12px;overflow-x:auto;background:#ffffff;"><div style="padding:10px 14px;font-weight:700;color:#0f172a;background:#f8fafc;border-bottom:1px solid #e2e8f0;font-size:15px;">FINAL REPORT - SPLIT-RATIO ROBUSTNESS VALIDATION</div><table class="dataframe">
  <thead>
