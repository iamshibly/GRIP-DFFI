...
LR = 1e-3
BATCH_SIZE = 128
LABEL_NOISE_ARTIFACT_DIR = "artifacts_label_noise_robustness"
LABEL_NOISE_CONFIGS = [
    ("label_noise_05", 0.05),
    ("label_noise_10", 0.10),
    ("label_noise_30", 0.30),
...
PAUSE_SECONDS_BETWEEN_VARIANTS = 5
os.makedirs(LABEL_NOISE_ARTIFACT_DIR, exist_ok=True)
...
def inject_label_noise(y, rate, seed):
    rng = np.random.default_rng(seed)
    y = np.asarray(y).astype(int).copy()
    classes = np.unique(y)
    if len(classes) < 2 or rate <= 0:
        return y, 0, int(len(y)), 0.0
    n_flip = int(round(float(rate) * len(y)))
    ...
    return y, int(n_flip), int(len(y)), float(n_flip / max(len(y), 1))


def rebuild_noisy_feature_inputs(stage, y_noisy, mi_pre=120):
    st = dict(stage)
    st["ytr"] = np.asarray(y_noisy).astype(int)
    st["class_dist_train"] = pd.Series(st["ytr"]).value_counts().sort_index().to_dict()
    num2 = list(st["num_cols_post"])
    cat2 = list(st["cat_cols_post"])
    Xtr = st["Xtr"]
    feat_names, mi_scores = compute_mi(Xtr, st["ytr"], num2, cat2)
    mi_pre_dyn = max(mi_pre, int(0.80 * len(feat_names))) if feat_names else 0
    if len(feat_names) > mi_pre_dyn > 0:
        ord_ = np.argsort(mi_scores)[::-1][:mi_pre_dyn]
        feat_names = [feat_names[i] for i in ord_]
    ...
    nc_sel = [c for c in num2 if c in fn_sel]
    cc_sel = [c for c in cat2 if c in fn_sel]
    Xnode, A = build_feature_graph(Xtr[fn_sel], st["ytr"], fn_sel, nc_sel, cc_sel)
    feat_keys = [feature_key(f, "num" if f in nc_sel else "cat") for f in fn_sel]
    feat_type_map = {k: {"name": feature_name_from_key(k), "kind": feature_kind_from_key(k)} for k in feat_keys}
    feat_key_to_mi = {feat_keys[i]: float(mi_scores[i]) for i in range(len(feat_keys))}
    st["feat_names"] = fn_sel
    st["feat_keys"] = feat_keys
    st["feat_type_map"] = feat_type_map
    st["local_mi_scores"] = np.asarray([feat_key_to_mi[k] for k in feat_keys], dtype=np.float32)
    st["Xnode"] = Xnode
    st["A"] = A
    st.pop("global_relevance_scores", None)
    st.pop("combined_scores", None)
    st.pop("selected_features", None)
    st.pop("selected_feature_keys", None)
    st.pop("global_relevance_map", None)
    return st


def make_label_noise_stage(stage, rate, seed):
    y_noisy, changed, total, actual_rate = inject_label_noise(stage["ytr"], rate, seed)
    st = rebuild_noisy_feature_inputs(stage, y_noisy, mi_pre=120)
    st["label_noise_changed"] = int(changed)
    st["label_noise_total"] = int(total)
    st["label_noise_actual_rate"] = float(actual_rate)
    return st
...
def finalize_stage_record_silent(stage, universality, occurrence, shared_key_to_id):
    ...
    meta = {
        ...
        "client_sizes": [len(x) for x in client_idx],
        "split_shapes": {"train": int(len(tr_ds)), "val": int(len(va_ds)), "test": int(len(te_ds))},
        "label_noise_changed": int(stage.get("label_noise_changed", 0)),
        "label_noise_total": int(stage.get("label_noise_total", len(ytr))),
        "label_noise_actual_rate": float(stage.get("label_noise_actual_rate", 0.0)),
    }
    return meta, tr_ds, va_ds, te_ds, client_idx


def new_model_for_label_noise_validation(meta, shared_feature_count):
...
def add_weighted_state(accum_state, state, weight):
    weight = float(weight)
    if accum_state is None:
        return {k: v.detach().cpu().clone() * weight for k, v in state.items()}
    for k, v in state.items():
        accum_state[k].add_(v.detach().cpu(), alpha=weight)
    return accum_state
...
def train_federated_model_for_label_noise_validation(meta_list, train_sets, val_sets, client_indices, shared_feature_count):
    seed_model = new_model_for_label_noise_validation(meta_list[0], shared_feature_count)
    shared_global_state = get_shared_state(seed_model)
    del seed_model
    private_dataset_states = []
    for meta in meta_list:
        m = new_model_for_label_noise_validation(meta, shared_feature_count)
        private_dataset_states.append(get_private_state(m))
        del m
        release_memory()
    best_bundle = {
        "global_val_acc": -1.0,
        "shared_state": clone_state_dict(shared_global_state),
        "private_states": [clone_state_dict(p) for p in private_dataset_states],
    }
    total_shared_size = float(sum(sum(len(idx) for idx in cidx) for cidx in client_indices))
    private_total_sizes = [float(sum(len(idx) for idx in cidx)) for cidx in client_indices]
    for _ in range(1, ROUNDS + 1):
        shared_accum = None
        private_accum = [None for _ in meta_list]
        for di, (meta, tr_ds, cidx) in enumerate(zip(meta_list, train_sets, client_indices)):
            for idx in cidx:
                ...
                local_ds = Subset(tr_ds, idx_list)
                model = new_model_for_label_noise_validation(meta, shared_feature_count)
                ...
                local_n = float(len(local_ds))
                shared_state = get_shared_state(model)
                private_state = get_private_state(model)
                shared_accum = add_weighted_state(shared_accum, shared_state, local_n / max(total_shared_size, 1.0))
                private_accum[di] = add_weighted_state(private_accum[di], private_state, local_n / max(private_total_sizes[di], 1.0))
                del shared_state, private_state, model, local_ds
                release_memory()
        del shared_global_state, private_dataset_states
        shared_global_state = shared_accum
        private_dataset_states = private_accum
        del shared_accum, private_accum
        release_memory()
        val_ns = []
        val_ms = []
        for di, meta in enumerate(meta_list):
            ...
            load_shared_state(model, shared_global_state)
            load_private_state(model, private_dataset_states[di])
            yv, pv = predict(model, mk_loader(val_sets[di], shuffle=False, batch_size=BATCH_SIZE))
...
def evaluate_clean_splits_for_label_noise(meta_list, val_sets, test_sets, best_bundle, shared_feature_count, variant_name, target_rate):
    rows = []
    val_metrics_each = []
    test_metrics_each = []
    val_ns = []
    test_ns = []
    total_changed = sum(int(meta.get("label_noise_changed", 0)) for meta in meta_list)
    total_train = sum(int(meta.get("label_noise_total", 0)) for meta in meta_list)
    actual_rate_global = float(total_changed / max(total_train, 1))
    for di, meta in enumerate(meta_list):
        model = new_model_for_label_noise_validation(meta, shared_feature_count)
        load_shared_state(model, best_bundle["shared_state"])
        load_private_state(model, best_bundle["private_states"][di])
        yv, pv = predict(model, mk_loader(val_sets[di], shuffle=False, batch_size=BATCH_SIZE))
        yt, pt = predict(model, mk_loader(test_sets[di], shuffle=False, batch_size=BATCH_SIZE))
        mv = compute_metrics(yv, pv)
        mt = compute_metrics(yt, pt)
        rows.append({"variant": variant_name, "target_label_noise_rate": float(target_rate), "actual_label_noise_rate": float(meta.get("label_noise_actual_rate", 0.0)), "noisy_train_labels": int(meta.get("label_noise_changed", 0)), "split": "VAL", "dataset": meta["name"], **mv})
        rows.append({"variant": variant_name, "target_label_noise_rate": float(target_rate), "actual_label_noise_rate": float(meta.get("label_noise_actual_rate", 0.0)), "noisy_train_labels": int(meta.get("label_noise_changed", 0)), "split": "TEST", "dataset": meta["name"], **mt})
        val_metrics_each.append(mv)
        test_metrics_each.append(mt)
        val_ns.append(len(yv))
        test_ns.append(len(yt))
        del model
        release_memory()
    all_metric_keys = sorted(set().union(*[m.keys() for m in val_metrics_each], *[m.keys() for m in test_metrics_each]))
    rows.append({"variant": variant_name, "target_label_noise_rate": float(target_rate), "actual_label_noise_rate": actual_rate_global, "noisy_train_labels": int(total_changed), "split": "VAL", "dataset": "global_weighted", **{k: weighted_metric(val_metrics_each, val_ns, k) for k in all_metric_keys}})
    rows.append({"variant": variant_name, "target_label_noise_rate": float(target_rate), "actual_label_noise_rate": actual_rate_global, "noisy_train_labels": int(total_changed), "split": "TEST", "dataset": "global_weighted", **{k: weighted_metric(test_metrics_each, test_ns, k) for k in all_metric_keys}})
    return rows
...
def render_final_table(df):
    ...
    column_order = ["variant", "target_label_noise_rate", "actual_label_noise_rate", "split", "dataset", "noisy_train_labels", "acc", "prec_macro", "rec_macro", "f1_macro", "logloss", "mcc", "kappa", "auc_roc_macro", "pr_auc", "delta_acc_vs_main", "delta_f1_vs_main", "delta_auc_vs_main", "prec_weighted", "rec_weighted", "f1_weighted", "auc_roc_micro", "auc_roc_weighted", "ppv_macro", "npv_macro", "ppv_weighted", "npv_weighted", "ppv_positive", "npv_negative"]
    view = view[[c for c in column_order if c in view.columns]]
    for c in view.columns:
        if c not in ["variant", "split", "dataset"]:
            if c == "noisy_train_labels":
    ...
    view.to_csv(os.path.join(LABEL_NOISE_ARTIFACT_DIR, "training_label_noise_robustness_final_report.csv"), index=False)
    if IN_NOTEBOOK and HTML is not None:
        ...
        html += '<div style="padding:10px 14px;font-weight:700;color:#0f172a;background:#f8fafc;border-bottom:1px solid #e2e8f0;font-size:15px;">FINAL REPORT - TRAINING LABEL-NOISE ROBUSTNESS VALIDATION</div>'
        html += table_html + '</div>'
        display(HTML(html))
    else:
...
def variant_report_path(variant_name):
    return os.path.join(LABEL_NOISE_ARTIFACT_DIR, f"training_label_noise_robustness_raw_{variant_name}.csv")


def collect_completed_variant_reports():
    parts = []
    for variant_name, _ in LABEL_NOISE_CONFIGS:
...
def run_one_label_noise_variant(variant_index, variant_name, noise_rate):
    reset_all_seeds(SEED + 1000 * (variant_index + 1))
    base_stage_records = []
    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        for spec in DATASET_SPECS:
            base_stage_records.append(prepare_dataset_for_feature_intelligence(spec, mi_pre=120))
    variant_stages = []
    for di, st in enumerate(base_stage_records):
        noisy_stage = make_label_noise_stage(st, noise_rate, SEED + 10000 * (variant_index + 1) + 97 * di)
        variant_stages.append(noisy_stage)
    del base_stage_records
    release_memory()
    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        _, universality, occurrence, shared_keys_global, shared_key_to_id = run_federated_feature_intelligence(variant_stages)
    all_meta = []
    all_tr = []
    all_va = []
    all_te = []
    ...
    del variant_stages, universality, occurrence, shared_key_to_id
    shared_feature_count = max(1, len(shared_keys_global))
    del shared_keys_global
    release_memory()
    best_bundle = train_federated_model_for_label_noise_validation(all_meta, all_tr, all_va, all_cidx, shared_feature_count)
    rows = evaluate_clean_splits_for_label_noise(all_meta, all_va, all_te, best_bundle, shared_feature_count, variant_name, noise_rate)
    del all_meta, all_tr, all_va, all_te, all_cidx, best_bundle, shared_feature_count
    release_memory()
    return rows


def run_training_label_noise_robustness_validation():
    for variant_index, (variant_name, noise_rate) in enumerate(LABEL_NOISE_CONFIGS):
        out_path = variant_report_path(variant_name)
        if SKIP_COMPLETED_VARIANTS and os.path.exists(out_path):
            release_memory()
            continue
        rows = run_one_label_noise_variant(variant_index, variant_name, noise_rate)
        pd.DataFrame(rows).to_csv(out_path, index=False)
        del rows
        release_memory()
        if variant_index < len(LABEL_NOISE_CONFIGS) - 1 and PAUSE_SECONDS_BETWEEN_VARIANTS > 0:
            time.sleep(PAUSE_SECONDS_BETWEEN_VARIANTS)
            release_memory()
    report = collect_completed_variant_reports()
    if report.empty:
        raise RuntimeError("No label-noise variant reports found.")
    report.to_csv(os.path.join(LABEL_NOISE_ARTIFACT_DIR, "training_label_noise_robustness_final_report_raw.csv"), index=False)
    report = add_main_deltas(report)
    report.to_csv(os.path.join(LABEL_NOISE_ARTIFACT_DIR, "training_label_noise_robustness_final_report_with_deltas.csv"), index=False)
...
if __name__ == "__main__":
    run_training_label_noise_robustness_validation()
```


<div style="margin:12px 0 18px 0;border:1px solid #dbe4ee;border-radius:12px;overflow-x:auto;background:#ffffff;"><div style="padding:10px 14px;font-weight:700;color:#0f172a;background:#f8fafc;border-bottom:1px solid #e2e8f0;font-size:15px;">FINAL REPORT - TRAINING LABEL-NOISE ROBUSTNESS VALIDATION</div><table class="dataframe">
  <thead>
    <tr style="text-align: right;">
      <th>variant</th>
      <th>target_label_noise_rate</th>
      <th>actual_label_noise_rate</th>
      <th>split</th>
      <th>dataset</th>
      <th>noisy_train_labels</th>
      <th>acc</th>
      <th>prec_macro</th>
      <th>rec_macro</th>
      <th>f1_macro</th>
