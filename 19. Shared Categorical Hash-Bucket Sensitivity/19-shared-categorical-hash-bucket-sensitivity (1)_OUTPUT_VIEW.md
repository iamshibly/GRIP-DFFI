...
LR = 1e-3
BATCH_SIZE = 256
HASH_BUCKET_ARTIFACT_DIR = "artifacts_hash_bucket_sensitivity"
HASH_BUCKET_CONFIGS = [1024, 8192]
os.makedirs(HASH_BUCKET_ARTIFACT_DIR, exist_ok=True)
...
def finalize_stage_record_silent(stage, universality, occurrence, shared_key_to_id):
    ...
    meta = {
        ...
        "client_sizes": [len(x) for x in client_idx],
        "split_shapes": {"train": int(len(tr_ds)), "val": int(len(va_ds)), "test": int(len(te_ds))},
        "hash_buckets": int(SHARED_HASH_BUCKETS),
        "shared_categorical_features": int(len(shared_cat_cols)),
    }
    return meta, tr_ds, va_ds, te_ds, client_idx


def new_model_for_hash_variant(meta, shared_feature_count):
...
def train_federated_model_for_hash_variant(meta_list, train_sets, val_sets, client_indices, shared_feature_count):
    seed_models = [new_model_for_hash_variant(meta, shared_feature_count) for meta in meta_list]
    ...
    for _ in range(1, ROUNDS + 1):
        ...
        for di, (meta, tr_ds, cidx) in enumerate(zip(meta_list, train_sets, client_indices)):
            for idx in cidx:
                ...
                model = new_model_for_hash_variant(meta, shared_feature_count)
...
def evaluate_hash_variant(hash_buckets, meta_list, val_sets, test_sets, best_bundle, shared_feature_count):
    rows = []
    val_metrics_each = []
    test_metrics_each = []
    val_ns = []
    ...
    for di, meta in enumerate(meta_list):
        ...
        rows.append({"variant": f"hash_{hash_buckets}", "hash_buckets": int(hash_buckets), "split": "VAL", "dataset": meta["name"], "shared_categorical_features": int(meta.get("shared_categorical_features", 0)), **mv})
        rows.append({"variant": f"hash_{hash_buckets}", "hash_buckets": int(hash_buckets), "split": "TEST", "dataset": meta["name"], "shared_categorical_features": int(meta.get("shared_categorical_features", 0)), **mt})
    ...
    rows.append({
        "variant": f"hash_{hash_buckets}",
        "hash_buckets": int(hash_buckets),
        "split": "VAL",
        "dataset": "global_weighted",
        "shared_categorical_features": int(sum(int(m.get("shared_categorical_features", 0)) for m in meta_list)),
...
def render_final_table(df):
    rename_map = {
        ...
        "ppv_weighted_ovr": "ppv_weighted",
        "npv_weighted_ovr": "npv_weighted",
        "delta_auc_roc_macro_ovr_vs_main": "delta_auc_vs_main",
        "delta_f1_macro_vs_main": "delta_f1_vs_main",
        "delta_acc_vs_main": "delta_acc_vs_main",
    }
    view = df.rename(columns=rename_map).copy()
    column_order = [
        "variant", "hash_buckets", "split", "dataset", "shared_categorical_features",
        "acc", "prec_macro", "rec_macro", "f1_macro", "logloss", "mcc", "kappa", "auc_roc_macro", "pr_auc",
        "delta_acc_vs_main", "delta_f1_vs_main", "delta_auc_vs_main",
        "prec_weighted", "rec_weighted", "f1_weighted", "auc_roc_micro", "auc_roc_weighted",
        "ppv_macro", "npv_macro", "ppv_weighted", "npv_weighted", "ppv_positive", "npv_negative",
    ]
    view = view[[c for c in column_order if c in view.columns]]
    for c in view.columns:
        if c not in ["variant", "split", "dataset"]:
            if c in ["hash_buckets", "shared_categorical_features"]:
    ...
    view.to_csv(os.path.join(HASH_BUCKET_ARTIFACT_DIR, "hash_bucket_sensitivity_final_report.csv"), index=False)
    if IN_NOTEBOOK and HTML is not None:
        ...
        html += '<div style="padding:10px 14px;font-weight:700;color:#0f172a;background:#f8fafc;border-bottom:1px solid #e2e8f0;font-size:15px;">FINAL REPORT - SHARED CATEGORICAL HASH-BUCKET SENSITIVITY VALIDATION</div>'
        html += table_html + '</div>'
        display(HTML(html))
    else:
...
def run_one_hash_bucket_variant(hash_buckets, seed_offset):
    global SHARED_HASH_BUCKETS
    SHARED_HASH_BUCKETS = int(hash_buckets)
    ...
    best_bundle = train_federated_model_for_hash_variant(all_meta, all_tr, all_va, all_cidx, shared_feature_count)
    rows = evaluate_hash_variant(hash_buckets, all_meta, all_va, all_te, best_bundle, shared_feature_count)
    del stage_records, all_meta, all_tr, all_va, all_te, all_cidx, best_bundle
    release_memory()
    return rows


def run_hash_bucket_sensitivity_validation():
    all_rows = []
    for seed_offset, hash_buckets in enumerate(HASH_BUCKET_CONFIGS):
        rows = run_one_hash_bucket_variant(hash_buckets, seed_offset)
        all_rows.extend(rows)
    report = pd.DataFrame(all_rows)
    report.to_csv(os.path.join(HASH_BUCKET_ARTIFACT_DIR, "hash_bucket_sensitivity_final_report_raw.csv"), index=False)
    report = add_main_deltas(report)
    report.to_csv(os.path.join(HASH_BUCKET_ARTIFACT_DIR, "hash_bucket_sensitivity_final_report_with_deltas.csv"), index=False)
    render_final_table(report)
    return report


if __name__ == "__main__":
    run_hash_bucket_sensitivity_validation()
```


<div style="margin:12px 0 18px 0;border:1px solid #dbe4ee;border-radius:12px;overflow-x:auto;background:#ffffff;"><div style="padding:10px 14px;font-weight:700;color:#0f172a;background:#f8fafc;border-bottom:1px solid #e2e8f0;font-size:15px;">FINAL REPORT - SHARED CATEGORICAL HASH-BUCKET SENSITIVITY VALIDATION</div><table class="dataframe">
  <thead>
    <tr style="text-align: right;">
      ...
      <th>split</th>
      <th>dataset</th>
      <th>shared_categorical_features</th>
      <th>acc</th>
      <th>prec_macro</th>
      <th>rec_macro</th>
      <th>f1_macro</th>
