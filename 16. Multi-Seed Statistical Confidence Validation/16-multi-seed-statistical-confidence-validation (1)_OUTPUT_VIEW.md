...
PAUSE_SECONDS_BETWEEN_SEEDS = 5
MULTI_SEED_ARTIFACT_DIR = "artifacts_multi_seed"
CURRENT_SPLIT_RATIO = (0.70, 0.15, 0.15)
os.makedirs(MULTI_SEED_ARTIFACT_DIR, exist_ok=True)
...
def split_three_way(df, y_raw, seed=None):
    if seed is None:
        seed = SEED
    train_ratio, val_ratio, test_ratio = CURRENT_SPLIT_RATIO
    ...
    X_tr, X_tmp, y_tr_raw, y_tmp_raw = train_test_split(
        ...
        random_state=seed,
        stratify=safe_stratify_labels(y_raw),
    )
    test_share_inside_tmp = test_ratio / total_eval_ratio
    X_va, X_te, y_va_raw, y_te_raw = train_test_split(
        X_tmp,
        y_tmp_raw,
        test_size=test_share_inside_tmp,
        random_state=seed,
        stratify=safe_stratify_labels(y_tmp_raw),
    )
    return (
        X_tr.reset_index(drop=True), pd.Series(y_tr_raw).reset_index(drop=True),
        X_va.reset_index(drop=True), pd.Series(y_va_raw).reset_index(drop=True),
...
def cross_plan(Xtr, ytr, cat_cols, max_base=6, seed=None):
    if seed is None:
        seed = SEED
    rng = np.random.RandomState(seed)
    base = cat_cols[:min(max_base, len(cat_cols))]
...
def finalize_stage_record_silent(stage, universality, occurrence, shared_key_to_id):
    ...
    selected_cols = [feature_name_from_key(k) for k in selected_keys]
    shared_keys = [k for k in selected_keys if k in shared_key_to_id and universality.get(k, 0.0) >= UNIVERSALITY_THRESHOLD and occurrence.get(k, 0) >= MIN_SHARED_OCCURRENCE]
    private_keys = [k for k in selected_keys if k not in set(shared_keys)]
    shared_num_cols = [feature_name_from_key(k) for k in shared_keys if feature_kind_from_key(k) == "num"]
    shared_cat_cols = [feature_name_from_key(k) for k in shared_keys if feature_kind_from_key(k) == "cat"]
    private_num_cols = [feature_name_from_key(k) for k in private_keys if feature_kind_from_key(k) == "num"]
    ...
    n_clients = choose_num_clients(len(tr_ds), ytr)
    client_idx = make_client_indices(ytr, n_clients, alpha=CLIENT_DIRICHLET_ALPHA, seed=SEED + stage["spec"]["domain_id"])
    meta = {"name": stage["spec"]["name"], "domain_id": stage["spec"]["domain_id"], "target_col": stage["target_col"], "task_kind": stage["task_kind"], "class_names": stage["class_names"], "normal_index": stage["normal_index"], "n_classes": int(stage["n_classes"]), "selected_features": [feature_name_from_key(k) for k in selected_keys], "selected_feature_keys": selected_keys, "num_cols": shared_num_cols + private_num_cols, "cat_cols": shared_cat_cols + private_cat_cols, "shared_num_cols": shared_num_cols, "shared_cat_cols": shared_cat_cols, "private_num_cols": private_num_cols, "private_cat_cols": private_cat_cols, "shared_num_global_ids": [shared_key_to_id[feature_key(c, "num")] for c in shared_num_cols], "shared_cat_global_ids": [shared_key_to_id[feature_key(c, "cat")] for c in shared_cat_cols], "cards": cards, "n_clients": int(n_clients), "client_sizes": [len(x) for x in client_idx], "split_shapes": {"train": int(len(tr_ds)), "val": int(len(va_ds)), "test": int(len(te_ds))}}
    return meta, tr_ds, va_ds, te_ds, client_idx


def new_model_for_seed(meta, shared_feature_count):
...
def train_federated_model_for_seed(meta_list, train_sets, val_sets, client_indices, shared_feature_count):
    seed_models = [new_model_for_seed(meta, shared_feature_count) for meta in meta_list]
    ...
    for _ in range(1, ROUNDS + 1):
        ...
        for di, (meta, tr_ds, cidx) in enumerate(zip(meta_list, train_sets, client_indices)):
            for idx in cidx:
                ...
                model = new_model_for_seed(meta, shared_feature_count)
                ...
                gc.collect()
        shared_global_state = average_state_dicts(shared_candidates, [n / float(sum(shared_sizes)) for n in shared_sizes])
        private_dataset_states = [average_state_dicts(private_candidates[di], [n / float(sum(private_sizes[di])) for n in private_sizes[di]]) for di in range(len(meta_list))]
        val_ns = []
        val_ms = []
...
def evaluate_seed(seed, meta_list, val_sets, test_sets, best_bundle, shared_feature_count):
    rows = []
    val_metrics_each = []
    test_metrics_each = []
    val_ns = []
    test_ns = []
    val_loaders = [mk_loader(v, shuffle=False, batch_size=BATCH_SIZE) for v in val_sets]
    test_loaders = [mk_loader(t, shuffle=False, batch_size=BATCH_SIZE) for t in test_sets]
    for di, meta in enumerate(meta_list):
        model = new_model_for_seed(meta, shared_feature_count)
        load_shared_state(model, best_bundle["shared_state"])
        load_private_state(model, best_bundle["private_states"][di])
        yv, pv = predict(model, val_loaders[di])
        yt, pt = predict(model, test_loaders[di])
        mv = compute_metrics(yv, pv)
        mt = compute_metrics(yt, pt)
        rows.append({"seed": int(seed), "split": "VAL", "dataset": meta["name"], **mv})
        rows.append({"seed": int(seed), "split": "TEST", "dataset": meta["name"], **mt})
    ...
    rows.append({"seed": int(seed), "split": "VAL", "dataset": "global_weighted", **{k: weighted_metric(val_metrics_each, val_ns, k) for k in all_metric_keys}})
    rows.append({"seed": int(seed), "split": "TEST", "dataset": "global_weighted", **{k: weighted_metric(test_metrics_each, test_ns, k) for k in all_metric_keys}})
    return rows
...
def run_one_seed(seed):
    ...
    best_bundle = train_federated_model_for_seed(all_meta, all_tr, all_va, all_cidx, shared_feature_count)
    rows = evaluate_seed(seed, all_meta, all_va, all_te, best_bundle, shared_feature_count)
    del stage_records, all_meta, all_tr, all_va, all_te, all_cidx, best_bundle
    try:
        del universality, occurrence, shared_keys_global, shared_key_to_id, shared_feature_count
    except Exception:
        pass
...
def aggregate_seed_statistics(raw_df):
    metric_cols = [c for c in raw_df.columns if c not in {"seed", "split", "dataset"}]
    rows = []
    for (split_name, dataset_name), grp in raw_df.groupby(["split", "dataset"], sort=False):
        row = {"split": split_name, "dataset": dataset_name, "n_seeds": int(grp["seed"].nunique())}
        for metric in metric_cols:
            vals = pd.to_numeric(grp[metric], errors="coerce").dropna().to_numpy(dtype=float)
            if len(vals) == 0:
                mean = std = lo = hi = np.nan
            else:
                mean = float(np.mean(vals))
                std = float(np.std(vals, ddof=1)) if len(vals) > 1 else 0.0
                ci = float(1.96 * std / math.sqrt(len(vals))) if len(vals) > 1 else 0.0
                lo = mean - ci
                hi = mean + ci
...
def compact_metric_table(df, include_seed=False):
    metric_cols = ["acc", "precision_macro", "recall_macro", "f1_macro", "logloss", "mcc", "kappa", "auc_roc_macro_ovr", "pr_auc_macro", "precision_weighted", "recall_weighted", "f1_weighted", "auc_roc_micro_ovr", "auc_roc_weighted_ovr", "ppv_macro_ovr", "npv_macro_ovr", "ppv_weighted_ovr", "npv_weighted_ovr", "ppv_positive", "npv_negative"]
    ...
    base_cols = ["seed", "split", "dataset"] if include_seed else ["split", "dataset"]
    cols = [c for c in base_cols if c in df.columns] + [c for c in metric_cols if c in df.columns]
    view = df[cols].copy()
    view = view.rename(columns=rename_map)
    for c in view.columns:
        if c not in {"seed", "split", "dataset"}:
            view[c] = pd.to_numeric(view[c], errors="coerce").map(lambda x: "" if pd.isna(x) else f"{float(x):.4f}")
    return view


def render_per_seed_table(raw_report):
    view = raw_report[raw_report["split"] == "TEST"].copy()
    view = view.sort_values(["seed", "dataset"]).reset_index(drop=True)
    out = compact_metric_table(view, include_seed=True)
    out.to_csv(os.path.join(MULTI_SEED_ARTIFACT_DIR, "multi_seed_statistical_confidence_per_seed_table.csv"), index=False)
    if IN_NOTEBOOK and HTML is not None:
        table_html = out.to_html(index=False, escape=False, border=0)
        html = '<div style="margin:12px 0 18px 0;border:1px solid #dbe4ee;border-radius:12px;overflow-x:auto;background:#ffffff;">'
        html += '<div style="padding:10px 14px;font-weight:700;color:#0f172a;background:#f8fafc;border-bottom:1px solid #e2e8f0;font-size:15px;">FINAL REPORT - MULTI-SEED PER-SEED TEST METRICS</div>'
        html += table_html + '</div>'
        display(HTML(html))
    else:
        builtins.print(out.to_string(index=False))


def render_final_table(stats_df):
    metric_cols = ["acc", "precision_macro", "recall_macro", "f1_macro", "logloss", "mcc", "kappa", "auc_roc_macro_ovr", "pr_auc_macro", "precision_weighted", "recall_weighted", "f1_weighted", "auc_roc_micro_ovr", "auc_roc_weighted_ovr", "ppv_macro_ovr", "npv_macro_ovr", "ppv_weighted_ovr", "npv_weighted_ovr", "ppv_positive", "npv_negative"]
    rename_map = {"precision_macro": "prec_macro", "recall_macro": "rec_macro", "auc_roc_macro_ovr": "auc_roc_macro", "pr_auc_macro": "pr_auc", "precision_weighted": "prec_weighted", "recall_weighted": "rec_weighted", "auc_roc_micro_ovr": "auc_roc_micro", "auc_roc_weighted_ovr": "auc_roc_weighted", "ppv_macro_ovr": "ppv_macro", "npv_macro_ovr": "npv_macro", "ppv_weighted_ovr": "ppv_weighted", "npv_weighted_ovr": "npv_weighted"}
    view_rows = []
    filtered = stats_df[stats_df["split"] == "TEST"].copy()
    for _, r in filtered.iterrows():
        out = {"split": r["split"], "dataset": r["dataset"], "n_seeds": int(r["n_seeds"])}
        for metric in metric_cols:
            mean_col = f"{metric}_mean"
            std_col = f"{metric}_std"
            if mean_col in r.index:
                mean = r[mean_col]
                std = r[std_col]
                label = rename_map.get(metric, metric)
                out[label] = "" if pd.isna(mean) else f"{float(mean):.4f} +/- {float(std):.4f}"
        view_rows.append(out)
    view = pd.DataFrame(view_rows)
    view.to_csv(os.path.join(MULTI_SEED_ARTIFACT_DIR, "multi_seed_statistical_confidence_final_table.csv"), index=False)
    if IN_NOTEBOOK and HTML is not None:
        table_html = view.to_html(index=False, escape=False, border=0)
        html = '<div style="margin:12px 0 18px 0;border:1px solid #dbe4ee;border-radius:12px;overflow-x:auto;background:#ffffff;">'
        html += '<div style="padding:10px 14px;font-weight:700;color:#0f172a;background:#f8fafc;border-bottom:1px solid #e2e8f0;font-size:15px;">FINAL REPORT - MULTI-SEED STATISTICAL CONFIDENCE SUMMARY</div>'
        html += table_html + '</div>'
        display(HTML(html))
    else:
...
def seed_report_path(seed):
    return os.path.join(MULTI_SEED_ARTIFACT_DIR, f"multi_seed_statistical_confidence_raw_seed_{int(seed)}.csv")


def collect_completed_seed_reports():
    parts = []
    for seed in SEEDS:
        path = seed_report_path(seed)
    ...
    out = out.drop_duplicates(subset=["seed", "split", "dataset"], keep="last")
    return out


def run_multi_seed_statistical_confidence_validation():
    if RUN_MODE == "single_seed":
        run_seeds = [int(ACTIVE_SEED)]
    elif RUN_MODE == "all_seeds":
        run_seeds = [int(s) for s in SEEDS]
    elif RUN_MODE == "aggregate_only":
        run_seeds = []
    else:
        raise ValueError("RUN_MODE must be single_seed, all_seeds, or aggregate_only")

    for i, seed in enumerate(run_seeds):
        out_path = seed_report_path(seed)
        if SKIP_COMPLETED_SEEDS and os.path.exists(out_path):
            builtins.print(f"Seed {seed} already completed. Skipping.")
            release_memory()
            continue

        builtins.print(f"Running seed {seed} ({i + 1}/{len(run_seeds)})")
        rows = run_one_seed(seed)
        pd.DataFrame(rows).to_csv(out_path, index=False)
        del rows
        release_memory()
        builtins.print(f"Completed seed {seed}")

        if i < len(run_seeds) - 1 and PAUSE_SECONDS_BETWEEN_SEEDS > 0:
            time.sleep(PAUSE_SECONDS_BETWEEN_SEEDS)
            release_memory()

    raw_report = collect_completed_seed_reports()
    if raw_report.empty:
        raise RuntimeError("No completed seed reports found.")
    raw_report.to_csv(os.path.join(MULTI_SEED_ARTIFACT_DIR, "multi_seed_statistical_confidence_raw.csv"), index=False)
    stats_report = aggregate_seed_statistics(raw_report)
    stats_report.to_csv(os.path.join(MULTI_SEED_ARTIFACT_DIR, "multi_seed_statistical_confidence_statistics.csv"), index=False)
    render_per_seed_table(raw_report)
    render_final_table(stats_report)
    return raw_report, stats_report


if __name__ == "__main__":
    run_multi_seed_statistical_confidence_validation()
```
...
<div style="margin:12px 0 18px 0;border:1px solid #dbe4ee;border-radius:12px;overflow-x:auto;background:#ffffff;"><div style="padding:10px 14px;font-weight:700;color:#0f172a;background:#f8fafc;border-bottom:1px solid #e2e8f0;font-size:15px;">FINAL REPORT - MULTI-SEED PER-SEED TEST METRICS</div><table class="dataframe">
  <thead>
...
<div style="margin:12px 0 18px 0;border:1px solid #dbe4ee;border-radius:12px;overflow-x:auto;background:#ffffff;"><div style="padding:10px 14px;font-weight:700;color:#0f172a;background:#f8fafc;border-bottom:1px solid #e2e8f0;font-size:15px;">FINAL REPORT - MULTI-SEED STATISTICAL CONFIDENCE SUMMARY</div><table class="dataframe">
  <thead>
    <tr style="text-align: right;">
      <th>split</th>
      <th>dataset</th>
