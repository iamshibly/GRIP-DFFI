```python
# Kaggle baseline validation script
# Included: Centralized_Deep, FedAvg, FedPer, RF, ExtraTrees, XGBoost, SVM, MLP, LogReg, AdaBoost, HistGB, DecisionTree, GaussianNB, KNN
...
def concat_flat_view(view, split_name, n_global_ids):
    Xs, ys = [], []
    for v in view:
        X, y = dense_from_flat_ds(v[split_name], v['meta'], n_global_ids)
        Xs.append(X)
        ys.append(y)
...
def train_pytorch_dense_classifier(Xtr, ytr, Xva, Xte, hidden=(256, 128), epochs=2, lr=1e-3, seed=SEED):
    ...
    ds = torch.utils.data.TensorDataset(torch.tensor(Xtr, dtype=torch.float32), torch.tensor(ytr, dtype=torch.long))
    ...
    return pred_prob(Xva), pred_prob(Xte)

def topk_keys(keys, scores, frac=0.50, min_k=8):
    n = len(keys)
    if n == 0:
        return []
    k = max(1, min(n, max(min_k, int(np.ceil(frac * n)))))
    idx = np.argsort(np.nan_to_num(scores, nan=-1e9))[::-1][:k]
    return [keys[i] for i in idx]

def stage_numeric_matrix(stage, feat_names, max_rows=1500):
    X = stage['Xtr'][feat_names].copy()
    if len(X) > max_rows:
        rng = np.random.default_rng(SEED)
        idx = rng.choice(len(X), max_rows, replace=False)
        X = X.iloc[idx].reset_index(drop=True)
        y = np.asarray(stage['ytr'])[idx]
    else:
        X = X.reset_index(drop=True)
        y = np.asarray(stage['ytr'])
    cols = []
    for c in feat_names:
        if c in stage['num_cols_post']:
            vals = sanitize_numeric_1d(pd.to_numeric(X[c], errors='coerce').to_numpy())
        else:
            vals, _ = pd.factorize(X[c].astype(str).fillna('missing'))
            vals = vals.astype(np.float32)
        cols.append(vals.astype(np.float32))
    M = np.stack(cols, axis=1) if cols else np.zeros((len(X), 0), np.float32)
    return M, y

def relieff_scores(stage, max_rows=1200, k_neighbors=5):
    feat_names = stage['feat_names']
    X, y = stage_numeric_matrix(stage, feat_names, max_rows=max_rows)
    if X.shape[1] == 0:
        return np.array([], np.float32)
    ...
    n = len(y)
    dist = np.sqrt(((Xn[:, None, :] - Xn[None, :, :]) ** 2).sum(axis=2))
    np.fill_diagonal(dist, np.inf)
    scores = np.zeros(Xn.shape[1], np.float32)
    for i in range(n):
        ...
        if len(same) == 0 or len(diff) == 0:
            continue
        hit_idx = same[np.argsort(dist[i, same])[:min(k_neighbors, len(same))]]
        miss_idx = diff[np.argsort(dist[i, diff])[:min(k_neighbors, len(diff))]]
        hit_term = np.abs(Xn[i] - Xn[hit_idx]).mean(axis=0)
        miss_term = np.abs(Xn[i] - Xn[miss_idx]).mean(axis=0)
        scores += (miss_term - hit_term).astype(np.float32)
    scores /= max(n, 1)
    return np.nan_to_num(scores).astype(np.float32)

def univariate_scores(stage):
    feat_names = stage['feat_names']
    X = stage['Xtr'][feat_names].copy().reset_index(drop=True)
    y = np.asarray(stage['ytr'])
    if len(feat_names) == 0:
        return np.array([], np.float32)
    num_cols = [f for f in feat_names if f in stage['num_cols_post']]
    cat_cols = [f for f in feat_names if f in stage['cat_cols_post']]
    score_map = {}
    if num_cols:
        Xnum = np.stack([sanitize_numeric_1d(pd.to_numeric(X[c], errors='coerce').to_numpy()) for c in num_cols], axis=1)
        try:
            fs, _ = f_classif(Xnum, y)
            fs = np.nan_to_num(fs)
        except Exception:
            fs = np.zeros(len(num_cols), np.float32)
        for c, s in zip(num_cols, fs):
            score_map[c] = float(max(0.0, s))
    if cat_cols:
        Xcat_cols = []
        for c in cat_cols:
            vals, _ = pd.factorize(X[c].astype(str).fillna('missing'))
            vals = vals.astype(np.float32)
            vals = vals - vals.min()
            Xcat_cols.append(vals)
        Xcat = np.stack(Xcat_cols, axis=1)
        try:
            cs, _ = chi2(Xcat, y)
            cs = np.nan_to_num(cs)
        except Exception:
            cs = np.zeros(len(cat_cols), np.float32)
        for c, s in zip(cat_cols, cs):
            score_map[c] = float(max(0.0, s))
    return np.array([score_map.get(f, 0.0) for f in feat_names], np.float32)

def ranking_map_mi_only(stage_records):
    out = {}
    for st in stage_records:
        out[st['spec']['name']] = topk_keys(st['feat_keys'], st['local_mi_scores'])
    return out

def ranking_map_graph_local(stage_records):
    out = {}
    for st in stage_records:
        _, scores = train_graph_refined_relevance_net(st['Xnode'], st['A'], init_state=None)
        out[st['spec']['name']] = topk_keys(st['feat_keys'], scores)
    return out

def ranking_map_graph_no_edges(stage_records):
    out = {}
    for st in stage_records:
        A0 = np.eye(len(st['feat_names']), dtype=np.float32)
        _, scores = train_graph_refined_relevance_net(st['Xnode'], A0, init_state=None)
        out[st['spec']['name']] = topk_keys(st['feat_keys'], scores)
    return out

def ranking_map_relieff(stage_records):
    out = {}
    for st in stage_records:
        scores = relieff_scores(st)
        out[st['spec']['name']] = topk_keys(st['feat_keys'], scores)
    return out

def ranking_map_univariate(stage_records):
    out = {}
    for st in stage_records:
        scores = univariate_scores(st)
        out[st['spec']['name']] = topk_keys(st['feat_keys'], scores)
    return out

def run_ranking_validation(name, stage_records, selected_key_map):
    flat_view, n_global_ids = build_global_flat_view(stage_records, selected_key_map)
    ...
    pva, pte = train_pytorch_dense_classifier(Xtr, ytr, Xva_s, Xte_s, hidden=(256, 128), epochs=2, lr=1e-3, seed=SEED)
    return {
        'method': name,
        ...
        'selected_counts': {st['spec']['name']: len(selected_key_map[st['spec']['name']]) for st in stage_records},
    }
    ...
    for si, st in enumerate(stage_records):
        ...
        if method_name == 'RF':
            clf = RandomForestClassifier(
                n_estimators=220, max_depth=None, min_samples_leaf=1,
                random_state=SEED, n_jobs=-1, class_weight='balanced_subsample'
            )
        elif method_name == 'ExtraTrees':
            clf = ExtraTreesClassifier(
                n_estimators=320, max_depth=None, min_samples_leaf=1,
            ...
            if not HAS_XGB:
                return None
            clf = XGBClassifier(
                n_estimators=220, max_depth=7, learning_rate=0.05,
            ...
            clf = AdaBoostClassifier(
                ...
                n_estimators=180, learning_rate=0.08, random_state=SEED
            )
        elif method_name == 'HistGB':
            clf = HistGradientBoostingClassifier(
                max_iter=220, learning_rate=0.06, max_depth=8, random_state=SEED
            )
        elif method_name == 'DecisionTree':
            clf = DecisionTreeClassifier(max_depth=18, min_samples_leaf=2, class_weight='balanced', random_state=SEED)
        elif method_name == 'GaussianNB':
            clf = GaussianNB()
        elif method_name == 'KNN':
            clf = KNeighborsClassifier(n_neighbors=7, weights='distance', metric='minkowski', p=2, n_jobs=-1)
        else:
...
results = []
methods = [
    ('MI_Only_Ranking', ranking_map_mi_only(stage_records)),
    ('GraphRefined_Local', ranking_map_graph_local(stage_records)),
    ('GraphRefined_NoEdges', ranking_map_graph_no_edges(stage_records)),
    ('ReliefF_Ranking', ranking_map_relieff(stage_records)),
    ('ChiSquare_ANOVA_Univariate', ranking_map_univariate(stage_records)),
]
for name, selected_key_map in methods:
    results.append(run_ranking_validation(name, stage_records, selected_key_map))

sel_rows = []
for res in results:
    row = {'method': res['method']}
    row.update(res['selected_counts'])
    sel_rows.append(row)
sel_df = pd.DataFrame(sel_rows)
show_table(
    'FEATURE-RANKING VALIDATION — SELECTED FEATURE COUNTS',
    sel_df,
    index=False,
    float_decimals=0,
)
...
show_table(
    'FEATURE-RANKING MECHANISM VALIDATION — GLOBAL METRICS',
    report,
    index=False,
    columns=cols,
...
plot_overlay(results, 'val', 'roc', os.path.join(ART_DIR, 'plots', 'feature_ranking_val_roc.png'), 'Feature-Ranking Validation — Validation ROC-AUC')
plot_overlay(results, 'test', 'roc', os.path.join(ART_DIR, 'plots', 'feature_ranking_test_roc.png'), 'Feature-Ranking Validation — Test ROC-AUC')
plot_overlay(results, 'val', 'pr', os.path.join(ART_DIR, 'plots', 'feature_ranking_val_pr.png'), 'Feature-Ranking Validation — Validation PR-AUC')
plot_overlay(results, 'test', 'pr', os.path.join(ART_DIR, 'plots', 'feature_ranking_test_pr.png'), 'Feature-Ranking Validation — Test PR-AUC')

metrics_json = {res['method']: {'val_metrics': res['val_metrics'], 'test_metrics': res['test_metrics'], 'selected_counts': res['selected_counts']} for res in results}
curves_json = {res['method']: {'val_curve': res['val_curve'], 'test_curve': res['test_curve']} for res in results}

with open(os.path.join(ART_DIR, 'feature_ranking_validation_metrics.json'), 'w') as f:
    json.dump(metrics_json, f, indent=2)
with open(os.path.join(ART_DIR, 'feature_ranking_validation_roc_pr_curves.json'), 'w') as f:
    json.dump(curves_json, f, indent=2)

print(os.path.join(ART_DIR, 'feature_ranking_validation_metrics.json'))
print(os.path.join(ART_DIR, 'feature_ranking_validation_roc_pr_curves.json'))
```
...
<div class="grip-table-block"><div class="grip-table-title">FEATURE-RANKING VALIDATION — SELECTED FEATURE COUNTS</div><table class="dataframe">
  <thead>
  ...
  <tbody>
    ...
    <tr>
      <td>ChiSquare_ANOVA_Univariate</td>
      <td>26</td>
      <td>23</td>
      <td>15</td>
      <td>29</td>
...
<div class="grip-table-block"><div class="grip-table-title">FEATURE-RANKING MECHANISM VALIDATION — GLOBAL METRICS</div><table class="dataframe">
  <thead>
    <tr style="text-align: right;">
      <th>split</th>
      <th>method</th>
...
![png](6-feature-ranking-mechanism-validation%20%281%29_OUTPUT_VIEW_files/6-feature-ranking-mechanism-validation%20%281%29_OUTPUT_VIEW_0_3.png)
    



    
![png](6-feature-ranking-mechanism-validation%20%281%29_OUTPUT_VIEW_files/6-feature-ranking-mechanism-validation%20%281%29_OUTPUT_VIEW_0_4.png)
    



    
![png](6-feature-ranking-mechanism-validation%20%281%29_OUTPUT_VIEW_files/6-feature-ranking-mechanism-validation%20%281%29_OUTPUT_VIEW_0_5.png)
    



    
![png](6-feature-ranking-mechanism-validation%20%281%29_OUTPUT_VIEW_files/6-feature-ranking-mechanism-validation%20%281%29_OUTPUT_VIEW_0_6.png)
    


    artifacts_baseline_validation_more_classical_kaggle/feature_ranking_validation_metrics.json
    artifacts_baseline_validation_more_classical_kaggle/feature_ranking_validation_roc_pr_curves.json
