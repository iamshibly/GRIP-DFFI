...
def cross_plan(Xtr, ytr, cat_cols, max_base=6, seed=SEED):
    ...
    if CROSS_FEATURE_MODE == "no_cross_features":
        return {"mode": CROSS_FEATURE_MODE, "use_triples": False, "base_cols": []}
    if CROSS_FEATURE_MODE == "pairwise_cross_only":
        return {"mode": CROSS_FEATURE_MODE, "use_triples": False, "base_cols": base}
    raise ValueError(f"Unknown CROSS_FEATURE_MODE: {CROSS_FEATURE_MODE}")


def apply_crosses(X, plan):
    X = X.copy()
    base = plan.get("base_cols", [])
    new = []
    if plan.get("mode") == "no_cross_features" or len(base) < 2:
        return X, new
...
LR = 1e-3
BATCH_SIZE = 256
CROSS_FEATURE_ARTIFACT_DIR = "artifacts_cross_feature_interaction"
CROSS_FEATURE_CONFIGS = ["no_cross_features", "pairwise_cross_only"]
CROSS_FEATURE_MODE = "pairwise_cross_only"
os.makedirs(CROSS_FEATURE_ARTIFACT_DIR, exist_ok=True)
...
def train_federated_model_for_cross_variant(meta_list, train_sets, val_sets, client_indices, shared_feature_count):
    seed_models = [new_model_for_cross_variant(meta, shared_feature_count) for meta in meta_list]
    ...
    for _ in range(1, ROUNDS + 1):
        ...
        for di, (meta, tr_ds, cidx) in enumerate(zip(meta_list, train_sets, client_indices)):
            for idx in cidx:
                ...
                model = new_model_for_cross_variant(meta, shared_feature_count)
...
def evaluate_cross_variant(variant, meta_list, val_sets, test_sets, best_bundle, shared_feature_count):
    rows = []
    val_metrics_each = []
    test_metrics_each = []
    val_ns = []
    ...
    for di, meta in enumerate(meta_list):
        ...
        rows.append({"variant": variant, "split": "VAL", "dataset": meta["name"], "cross_features_generated": int(meta.get("cross_features_generated", 0)), **mv})
        rows.append({"variant": variant, "split": "TEST", "dataset": meta["name"], "cross_features_generated": int(meta.get("cross_features_generated", 0)), **mt})
    ...
    rows.append({"variant": variant, "split": "VAL", "dataset": "global_weighted", "cross_features_generated": int(sum(int(m.get("cross_features_generated", 0)) for m in meta_list)), **{k: weighted_metric(val_metrics_each, val_ns, k) for k in all_metric_keys}})
    rows.append({"variant": variant, "split": "TEST", "dataset": "global_weighted", "cross_features_generated": int(sum(int(m.get("cross_features_generated", 0)) for m in meta_list)), **{k: weighted_metric(test_metrics_each, test_ns, k) for k in all_metric_keys}})
    return rows
...
def add_main_deltas(report):
    ...
    if ref is None or "dataset" not in ref.columns or "split" not in ref.columns:
        return report
    cols = [c for c in ["acc", "f1_macro", "auc_roc_macro_ovr"] if c in ref.columns]
    if not cols:
        return report
...
def render_final_table(df):
    ...
    view.to_csv(os.path.join(CROSS_FEATURE_ARTIFACT_DIR, "cross_feature_interaction_final_report.csv"), index=False)
    if IN_NOTEBOOK and HTML is not None:
        ...
        html += '<div style="padding:10px 14px;font-weight:700;color:#0f172a;background:#f8fafc;border-bottom:1px solid #e2e8f0;font-size:15px;">FINAL REPORT - CROSS-FEATURE INTERACTION NECESSITY VALIDATION</div>'
        html += table_html + '</div>'
        display(HTML(html))
    else:
...
def run_one_cross_feature_variant(variant, seed_offset):
    global CROSS_FEATURE_MODE
    CROSS_FEATURE_MODE = variant
    ...
    best_bundle = train_federated_model_for_cross_variant(all_meta, all_tr, all_va, all_cidx, shared_feature_count)
    rows = evaluate_cross_variant(variant, all_meta, all_va, all_te, best_bundle, shared_feature_count)
    del stage_records, all_meta, all_tr, all_va, all_te, all_cidx, best_bundle
    release_memory()
    return rows


def run_cross_feature_interaction_necessity_validation():
    all_rows = []
    for seed_offset, variant in enumerate(CROSS_FEATURE_CONFIGS):
        rows = run_one_cross_feature_variant(variant, seed_offset)
        all_rows.extend(rows)
    report = pd.DataFrame(all_rows)
    report.to_csv(os.path.join(CROSS_FEATURE_ARTIFACT_DIR, "cross_feature_interaction_final_report_raw.csv"), index=False)
    report = add_main_deltas(report)
    report.to_csv(os.path.join(CROSS_FEATURE_ARTIFACT_DIR, "cross_feature_interaction_final_report_with_deltas.csv"), index=False)
    render_final_table(report)
    return report


if __name__ == "__main__":
    run_cross_feature_interaction_necessity_validation()
```


<div style="margin:12px 0 18px 0;border:1px solid #dbe4ee;border-radius:12px;overflow-x:auto;background:#ffffff;"><div style="padding:10px 14px;font-weight:700;color:#0f172a;background:#f8fafc;border-bottom:1px solid #e2e8f0;font-size:15px;">FINAL REPORT - CROSS-FEATURE INTERACTION NECESSITY VALIDATION</div><table class="dataframe">
  <thead>
    <tr style="text-align: right;">
      <th>variant</th>
      <th>split</th>
      <th>dataset</th>
      <th>cross_features_generated</th>
      <th>acc</th>
      <th>prec_macro</th>
      <th>rec_macro</th>
      <th>f1_macro</th>
