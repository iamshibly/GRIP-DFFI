...
class DiffusionRegularizer(nn.Module):
    ...
    def __init__(self, d_model=192, n_steps=None, hidden=256, noise_scale=1.0):
        super().__init__()
        if n_steps is None:
            n_steps = int(globals().get('ACTIVE_DIFFUSION_STEPS', DIFFUSION_STEPS))
        self.d_model = d_model
        self.n_steps = max(2, int(n_steps))
        self.noise_scale = float(noise_scale)
        beta = torch.linspace(0.0001, 0.02, self.n_steps)
        alpha = 1.0 - beta
        alpha_bar = torch.cumprod(alpha, dim=0)
        self.register_buffer('alpha_bar', alpha_bar)
        self.t_emb = nn.Embedding(self.n_steps, d_model)
...
LR = 1e-3
BATCH_SIZE = 256
OUTPUT_DIR = "grip_dffi_diffusion_regularized_shared_backbone_validation_outputs"
CURRENT_CFG = {}
...
class VariantGRIPDFFIModel(nn.Module):
    def __init__(self, meta, n_shared_feature_ids, use_custom_backbone=True, use_diffusion=True):
        ...
        self.head = PrivateHead(d=D_MODEL, n_classes=meta["n_classes"], drop=DROP)
        self.diffusion = DiffusionRegularizer(d_model=D_MODEL, n_steps=int(globals().get('ACTIVE_DIFFUSION_STEPS', DIFFUSION_STEPS)), noise_scale=float(globals().get('ACTIVE_DIFFUSION_NOISE_SCALE', 1.0))) if use_diffusion else None
        self.register_buffer("shared_num_ids", torch.tensor(meta["shared_num_global_ids"], dtype=torch.long))
        self.register_buffer("shared_cat_ids", torch.tensor(meta["shared_cat_global_ids"], dtype=torch.long))
    def forward(self, xsn, xsc, xpn, xpc):
        stoks = self.shared_tokenizer(xsn, xsc, self.shared_num_ids, self.shared_cat_ids)
...
def make_model_factory(cfg):
    def factory(meta):
        return VariantGRIPDFFIModel(meta=meta, n_shared_feature_ids=max(1, globals().get("GLOBAL_SHARED_FEATURE_COUNT", 1)), use_custom_backbone=not cfg.get("no_customized_backbone", False), use_diffusion=not cfg.get("no_diffusion_backbone", False)).to(DEVICE)
    return factory
...
def run_variant(variant_name, cfg, specs):
    ...
    globals()['ACTIVE_DIFFUSION_STEPS'] = int(cfg.get('diffusion_steps', DIFFUSION_STEPS))
    globals()['ACTIVE_DIFFUSION_LOSS_WEIGHT'] = float(cfg.get('diffusion_loss_weight', DIFFUSION_LOSS_WEIGHT))
    globals()['ACTIVE_DIFFUSION_NOISE_SCALE'] = float(cfg.get('diffusion_noise_scale', 1.0))
    set_seed(SEED)
    ...
    for _ in range(ROUNDS):
        ...
        for di, (meta, tr_ds, client_idx) in enumerate(zip(all_meta, all_tr, all_cidx)):
            for idx in client_idx:
                ...
                train_client(model, local_loader, epochs=LOCAL_EPOCHS, lr=LR, diffusion_weight=float(cfg.get('diffusion_loss_weight', globals().get('ACTIVE_DIFFUSION_LOSS_WEIGHT', DIFFUSION_LOSS_WEIGHT))))
...
diffusion_regularized_shared_backbone_variants = [
    ("No-Diffusion Backbone", {"no_diffusion_backbone": True, "diffusion_loss_weight": 0.0}),
    ("Low Diffusion Loss Weight", {"diffusion_loss_weight": 0.01}),
    ("Medium Diffusion Loss Weight", {"diffusion_loss_weight": 0.05}),
    ("High Diffusion Loss Weight", {"diffusion_loss_weight": 0.20}),
    ("Fewer Diffusion Steps", {"diffusion_steps": 10, "diffusion_loss_weight": 0.05}),
    ("More Diffusion Steps", {"diffusion_steps": 40, "diffusion_loss_weight": 0.05}),
    ("High-Heterogeneity Diffusion Setting", {"client_dirichlet_alpha": 0.10, "diffusion_steps": 20, "diffusion_loss_weight": 0.05}),
    ("Noisy Diffusion Setting", {"diffusion_steps": 20, "diffusion_loss_weight": 0.05, "diffusion_noise_scale": 1.75}),
]

specs = download_dataset_specs([dict(x) for x in DATASET_SPECS])
results = [run_variant(name, cfg, specs) for name, cfg in diffusion_regularized_shared_backbone_variants]
...
with open(os.path.join(OUTPUT_DIR, "grip_dffi_diffusion_regularized_shared_backbone_validation_metrics_and_curves.json"), "w") as f:
...
<table border="1" class="dataframe">
  ...
  <tbody>
    ...
    <tr>
      <th>1</th>
      <td>Low Diffusion Loss Weight</td>
      <td>0.9835</td>
      <td>0.9796</td>
      <td>0.9527</td>
      <td>0.9632</td>
    ...
    <tr>
      <th>2</th>
      <td>Medium Diffusion Loss Weight</td>
      <td>0.9833</td>
      <td>0.9796</td>
      <td>0.9518</td>
      <td>0.9628</td>
    ...
    <tr>
      <th>3</th>
      <td>High Diffusion Loss Weight</td>
      <td>0.9748</td>
      <td>0.9694</td>
      <td>0.9374</td>
      <td>0.9453</td>
    ...
    <tr>
      <th>6</th>
      <td>High-Heterogeneity Diffusion Setting</td>
      <td>0.9162</td>
      <td>0.4769</td>
      <td>0.5180</td>
      <td>0.4956</td>
...
![png](10-diffusion-regularized-shared-backbone-validati%20%281%29_OUTPUT_VIEW_files/10-diffusion-regularized-shared-backbone-validati%20%281%29_OUTPUT_VIEW_0_4.png)
    



    
![png](10-diffusion-regularized-shared-backbone-validati%20%281%29_OUTPUT_VIEW_files/10-diffusion-regularized-shared-backbone-validati%20%281%29_OUTPUT_VIEW_0_5.png)
    



    
![png](10-diffusion-regularized-shared-backbone-validati%20%281%29_OUTPUT_VIEW_files/10-diffusion-regularized-shared-backbone-validati%20%281%29_OUTPUT_VIEW_0_6.png)
    



    
![png](10-diffusion-regularized-shared-backbone-validati%20%281%29_OUTPUT_VIEW_files/10-diffusion-regularized-shared-backbone-validati%20%281%29_OUTPUT_VIEW_0_7.png)
