...
FAST_MAX_ROWS_PER_TABLE = 25000
FAST_MAX_TRAIN_ROWS = 18000
FAST_MAX_VAL_ROWS = 5000
FAST_MAX_TEST_ROWS = 5000
...
def combine_tables(files, force_cols=None, max_rows=None):
    if max_rows is None:
        max_rows = FAST_MAX_ROWS_PER_TABLE
    dfs = []
    for fp in files:
        try:
...
def subsample_xy_frame(X, y, limit):
    if limit is None or len(X) <= int(limit):
        return X.reset_index(drop=True), np.asarray(y)
    rng = np.random.RandomState(SEED)
    idx = np.sort(rng.choice(len(X), int(limit), replace=False))
...
def prepare_dataset_for_feature_intelligence(spec, mi_pre=160):
    ...
    df_train, y_train_raw = subsample_xy_frame(df_train, y_train_raw, FAST_MAX_TRAIN_ROWS)
    df_val, y_val_raw = subsample_xy_frame(df_val, y_val_raw, FAST_MAX_VAL_ROWS)
    df_test, y_test_raw = subsample_xy_frame(df_test, y_test_raw, FAST_MAX_TEST_ROWS)
...
LR = 1e-3
BATCH_SIZE = 256
OUTPUT_DIR = "grip_dffi_ablation_outputs"
...
def run_feature_intelligence_variant(stage_records, cfg):
    ...
    weights = [st["n_train"] for st in stage_records]
    weights = [w / float(sum(weights)) for w in weights]
    use_graph = not cfg.get("no_graph_refined_relevance", False)
    use_fs = not cfg.get("no_feature_selection", False)
    use_rl = not cfg.get("no_rl_subset_policy", False)
    ...
    for st in stage_records:
        ...
        if len(feat_keys) == 0:
            ...
            if not use_fs:
                ...
                selected = deterministic_top_subset(feat_names, combined, keep_ratio=0.70)
                selected_keys = [k for k in feat_keys if feature_name_from_key(k) in set(selected)]
...
def display_table(df, title):
    if IN_NOTEBOOK:
        try:
            show_table(title, df, index=False)
            return
        except Exception:
            pass
    show_table(title, df, index=False)
...
def run_variant(variant_name, cfg, specs):
    ...
    set_seed(SEED)
    mi_pre = 1000000 if cfg.get("no_feature_selection", False) else 160
...
OUTPUT_DIR = "grip_dffi_classical_machine_learning_validation_6models_fast_outputs"
os.makedirs(OUTPUT_DIR, exist_ok=True)
os.makedirs(os.path.join(OUTPUT_DIR, "plots"), exist_ok=True)
FEATURE_INTEL_ROUNDS = 2
ROUNDS = 15
...
class StandardFLBaselineModel(nn.Module):
    def __init__(self, meta, n_shared_feature_ids):
        super().__init__()
        self.shared_tokenizer = SharedRouteTokenizer(n_shared_feature_ids, hash_buckets=SHARED_HASH_BUCKETS, d_model=D_MODEL, dropout=DROP)
        self.backbone = SharedBackbone(d=D_MODEL, blocks=N_BLOCKS, heads=N_HEADS, ff=FF_DIM, drop=DROP)
        self.head = PrivateHead(d=D_MODEL, n_classes=meta["n_classes"], drop=DROP)
        self.register_buffer("shared_num_ids", torch.tensor(meta["shared_num_global_ids"], dtype=torch.long))
        self.register_buffer("shared_cat_ids", torch.tensor(meta["shared_cat_global_ids"], dtype=torch.long))
    def forward(self, xsn, xsc, xpn, xpc):
        stoks = self.shared_tokenizer(xsn, xsc, self.shared_num_ids, self.shared_cat_ids)
        z = self.backbone(stoks)
        logits = self.head(z)
        return logits, {"latent": z}

def make_baseline_model_factory(n_shared_feature_ids):
    def factory(meta):
        return StandardFLBaselineModel(meta=meta, n_shared_feature_ids=n_shared_feature_ids).to(DEVICE)
    return factory

def get_baseline_shared_state(model):
    st = {}
    st.update(_prefix_state('shared_tokenizer', cpu_state(model.shared_tokenizer)))
    st.update(_prefix_state('backbone', cpu_state(model.backbone)))
    return st

def load_baseline_shared_state(model, state):
    load_state(model.shared_tokenizer, _unprefix_state('shared_tokenizer', state))
    load_state(model.backbone, _unprefix_state('backbone', state))

def get_baseline_head_state(model):
    return _prefix_state('head', cpu_state(model.head))

def load_baseline_head_state(model, state):
    load_state(model.head, _unprefix_state('head', state))

def get_baseline_full_state(model):
    st = {}
    st.update(get_baseline_shared_state(model))
    st.update(get_baseline_head_state(model))
    return st

def load_baseline_full_state(model, state):
    load_baseline_shared_state(model, state)
    load_baseline_head_state(model, state)

def clone_tensor_dict(d):
    return {k: v.detach().cpu().clone() for k, v in d.items()}

def zero_like_param_state(model, shared_only=False):
    out = {}
    for name, p in model.named_parameters():
        if shared_only and name.startswith('head.'):
            continue
        out[name] = torch.zeros_like(p.detach().cpu())
    return out

def get_named_param_state(model, shared_only=False):
    out = {}
    for name, p in model.named_parameters():
        if shared_only and name.startswith('head.'):
            continue
...
def cosine_contrastive_loss(z, z_global, z_prev, temperature=MOON_TEMP):
    z = F.normalize(z, dim=1)
    z_global = F.normalize(z_global, dim=1)
    z_prev = F.normalize(z_prev, dim=1)
    pos = torch.sum(z * z_global, dim=1, keepdim=True)
    neg = torch.sum(z * z_prev, dim=1, keepdim=True)
    logits = torch.cat([pos, neg], dim=1) / float(temperature)
    targets = torch.zeros(z.size(0), dtype=torch.long, device=z.device)
    return F.cross_entropy(logits, targets)

def train_client_standard(model, loader, method, lr=LR, prox_mu=FEDPROX_MU, global_full_params=None, server_control=None, client_control=None, global_ref_model=None, prev_ref_model=None):
    model.train()
    opt = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=0.0001)
    step_count = 0
    global_params_dev = None
    server_control_dev = None
    client_control_dev = None
    if method == 'FedProx' and global_full_params is not None:
        global_params_dev = {k: v.to(DEVICE) for k, v in global_full_params.items()}
    if method == 'SCAFFOLD' and server_control is not None and client_control is not None:
        server_control_dev = {k: v.to(DEVICE) for k, v in server_control.items()}
        client_control_dev = {k: v.to(DEVICE) for k, v in client_control.items()}
    if global_ref_model is not None:
        global_ref_model.eval()
    if prev_ref_model is not None:
        prev_ref_model.eval()
    for _ in range(epochs := LOCAL_EPOCHS):
        for xsn, xsc, xpn, xpc, y in loader:
            xsn = xsn.to(DEVICE, non_blocking=True)
            xsc = xsc.to(DEVICE, non_blocking=True)
            xpn = xpn.to(DEVICE, non_blocking=True)
            ...
            logits, aux = model(xsn, xsc, xpn, xpc)
            loss = F.cross_entropy(logits, y)
            if method == 'FedProx' and global_params_dev is not None:
                prox = torch.tensor(0.0, device=DEVICE)
                for name, p in model.named_parameters():
                    prox = prox + torch.sum((p - global_params_dev[name]) ** 2)
                loss = loss + 0.5 * prox_mu * prox
            if method == 'SCAFFOLD' and server_control_dev is not None and client_control_dev is not None:
                corr = torch.tensor(0.0, device=DEVICE)
                for name, p in model.named_parameters():
                    if name.startswith('head.'):
                        continue
                    corr = corr + torch.sum(p * (server_control_dev[name] - client_control_dev[name]))
                loss = loss + corr
            if method == 'MOON' and global_ref_model is not None and prev_ref_model is not None:
                with torch.no_grad():
                    _, aux_g = global_ref_model(xsn, xsc, xpn, xpc)
                    _, aux_p = prev_ref_model(xsn, xsc, xpn, xpc)
                moon_loss = cosine_contrastive_loss(aux['latent'], aux_g['latent'], aux_p['latent'])
...
def fednova_aggregate(base_state, local_states, sample_sizes, local_steps):
    total = float(sum(sample_sizes))
    probs = [n / total for n in sample_sizes]
    tau_eff = sum(p * s for p, s in zip(probs, local_steps))
    out = {}
    for k in base_state:
        agg = sum(probs[i] * ((base_state[k] - local_states[i][k]) / float(max(local_steps[i], 1))) for i in range(len(local_states)))
        out[k] = base_state[k] - tau_eff * agg
    return out

def prepare_standard_fl_bundle(specs):
    global GLOBAL_SHARED_FEATURE_COUNT
    stage_records = [silent_call(prepare_dataset_for_feature_intelligence, spec, 160) for spec in specs]
    _, universality, occurrence, shared_keys_global, shared_key_to_id = silent_call(run_feature_intelligence_variant, stage_records, {"no_shared_private_branch": True})
    GLOBAL_SHARED_FEATURE_COUNT = max(1, len(shared_keys_global))
    final_bundle = [finalize_stage_record_variant(st, universality, occurrence, shared_key_to_id, {"no_shared_private_branch": True}) for st in stage_records]
    all_meta = [x[0] for x in final_bundle]
    all_tr = [x[1] for x in final_bundle]
    all_va = [x[2] for x in final_bundle]
    all_te = [x[3] for x in final_bundle]
    all_cidx = [x[4] for x in final_bundle]
    return all_meta, all_tr, all_va, all_te, all_cidx, GLOBAL_SHARED_FEATURE_COUNT
...
def run_standard_fl_baseline(method_name, specs):
    set_seed(SEED)
    all_meta, all_tr, all_va, all_te, all_cidx, n_shared = prepare_standard_fl_bundle(specs)
    model_factory = make_baseline_model_factory(n_shared)
    va_loaders = [mk_loader(v, shuffle=False, batch_size=BATCH_SIZE) for v in all_va]
    te_loaders = [mk_loader(t, shuffle=False, batch_size=BATCH_SIZE) for t in all_te]
    seed_models = [model_factory(meta) for meta in all_meta]
    shared_global_state = get_baseline_shared_state(seed_models[0])
    head_dataset_states = [get_baseline_head_state(m) for m in seed_models]
    client_controls = {}
    prev_local_states = {}
    if method_name == 'SCAFFOLD':
        tmp_model = model_factory(all_meta[0])
        server_control = zero_like_param_state(tmp_model, shared_only=True)
        del tmp_model
    else:
        server_control = None
    best_bundle = {"global_val_acc": -1.0, "shared_state": clone_tensor_dict(shared_global_state), "head_states": [clone_tensor_dict(h) for h in head_dataset_states]}
    for _ in range(ROUNDS):
        shared_candidates = []
        shared_sizes = []
        shared_steps = []
        head_candidates = defaultdict(list)
        head_sizes = defaultdict(list)
        head_steps = defaultdict(list)
        scaffold_updates = []
        for di, (meta, tr_ds, client_idx) in enumerate(zip(all_meta, all_tr, all_cidx)):
            for ci, idx in enumerate(client_idx):
                local_ds = tr_ds.subset(idx)
                local_loader = mk_loader(local_ds, shuffle=True, batch_size=BATCH_SIZE)
                client_key = f"{di}::{ci}"
                model = model_factory(meta)
                load_baseline_shared_state(model, shared_global_state)
                load_baseline_head_state(model, head_dataset_states[di])
                global_full_params = None
                if method_name == 'FedProx':
                    global_full_params = get_named_param_state(model, shared_only=False)
                global_ref_model = None
                prev_ref_model = None
                if method_name == 'MOON':
                    global_ref_model = model_factory(meta)
                    load_baseline_shared_state(global_ref_model, shared_global_state)
                    load_baseline_head_state(global_ref_model, head_dataset_states[di])
                    if client_key in prev_local_states:
                        prev_ref_model = model_factory(meta)
                        load_baseline_full_state(prev_ref_model, prev_local_states[client_key])
                if method_name == 'SCAFFOLD':
                    if client_key not in client_controls:
                        client_controls[client_key] = zero_like_param_state(model, shared_only=True)
                    init_shared_params = get_named_param_state(model, shared_only=True)
                    steps = train_client_standard(model, local_loader, method_name, lr=LR, server_control=server_control, client_control=client_controls[client_key])
                    final_shared_params = get_named_param_state(model, shared_only=True)
                    c_old = client_controls[client_key]
                    c_new = {}
                    for name in init_shared_params:
                        c_new[name] = c_old[name] - server_control[name] + (init_shared_params[name] - final_shared_params[name]) / float(LR * max(steps, 1))
                    client_controls[client_key] = c_new
                    scaffold_updates.append((client_key, len(local_ds)))
                else:
                    steps = train_client_standard(model, local_loader, method_name, lr=LR, global_full_params=global_full_params, global_ref_model=global_ref_model, prev_ref_model=prev_ref_model)
                shared_candidates.append(get_baseline_shared_state(model))
                shared_sizes.append(len(local_ds))
                shared_steps.append(steps)
                head_candidates[di].append(get_baseline_head_state(model))
                head_sizes[di].append(len(local_ds))
                head_steps[di].append(steps)
                if method_name == 'MOON':
                    prev_local_states[client_key] = get_baseline_full_state(model)
                del model, local_ds, local_loader
                if global_ref_model is not None:
                    del global_ref_model
                if prev_ref_model is not None:
                    del prev_ref_model
                if DEVICE == 'cuda':
                    torch.cuda.empty_cache()
                gc.collect()
        if method_name == 'FedNova':
            shared_global_state = fednova_aggregate(shared_global_state, shared_candidates, shared_sizes, shared_steps)
            new_head_states = []
            for di in range(len(all_meta)):
                new_head_states.append(fednova_aggregate(head_dataset_states[di], head_candidates[di], head_sizes[di], head_steps[di]))
            head_dataset_states = new_head_states
        else:
            sw = [n / float(sum(shared_sizes)) for n in shared_sizes]
            shared_global_state = average_state_dicts(shared_candidates, sw)
            new_head_states = []
            for di in range(len(all_meta)):
                w = [n / float(sum(head_sizes[di])) for n in head_sizes[di]]
                new_head_states.append(average_state_dicts(head_candidates[di], w))
            head_dataset_states = new_head_states
        if method_name == 'SCAFFOLD' and len(client_controls):
            all_keys = list(client_controls.keys())
            total_n = float(sum(n for _, n in scaffold_updates)) if len(scaffold_updates) else float(len(all_keys))
            ref = client_controls[all_keys[0]]
            server_control = {}
            for name in ref:
                acc = torch.zeros_like(ref[name])
                for ck, n in scaffold_updates:
                    acc = acc + (n / total_n) * client_controls[ck][name]
                server_control[name] = acc
        val_metrics_each = []
        val_ns = []
        for di, meta in enumerate(all_meta):
            model = model_factory(meta)
        ...
        global_val_acc = sum(m["acc"] * n for m, n in zip(val_metrics_each, val_ns)) / max(sum(val_ns), 1)
        if global_val_acc > best_bundle["global_val_acc"]:
            best_bundle = {"global_val_acc": float(global_val_acc), "shared_state": clone_tensor_dict(shared_global_state), "head_states": [clone_tensor_dict(h) for h in head_dataset_states]}
    best_shared_state = best_bundle["shared_state"]
    best_head_states = best_bundle["head_states"]
    val_preds = {}
    test_preds = {}
    val_metrics_each = []
    test_metrics_each = []
    for di, meta in enumerate(all_meta):
        model = model_factory(meta)
        load_baseline_shared_state(model, best_shared_state)
        load_baseline_head_state(model, best_head_states[di])
        yv, pv = predict(model, va_loaders[di])
        yt, pt = predict(model, te_loaders[di])
        val_preds[meta["name"]] = {"y": yv, "p": pv}
        test_preds[meta["name"]] = {"y": yt, "p": pt}
    ...
    val_row = evaluate_global_weighted(val_metrics_each, val_preds, all_meta, "VAL")
    test_row = evaluate_global_weighted(test_metrics_each, test_preds, all_meta, "TEST")
    val_row["variant"] = method_name
    test_row["variant"] = method_name
    yv_bin_all = []
    pv_bin_all = []
    yt_bin_all = []
    pt_bin_all = []
    ...
    if yv_bin_all:
        yv_bin = np.concatenate(yv_bin_all)
        pv_bin = np.concatenate(pv_bin_all, axis=0)
        val_roc = collect_roc_data(yv_bin, pv_bin, f"{method_name}_val_binary", ["Normal", "Attack"])
        val_pr = collect_pr_data(yv_bin, pv_bin, f"{method_name}_val_binary", positive_label="Attack")
    if yt_bin_all:
        yt_bin = np.concatenate(yt_bin_all)
        pt_bin = np.concatenate(pt_bin_all, axis=0)
        test_roc = collect_roc_data(yt_bin, pt_bin, f"{method_name}_test_binary", ["Normal", "Attack"])
        test_pr = collect_pr_data(yt_bin, pt_bin, f"{method_name}_test_binary", positive_label="Attack")
    return {"variant": method_name, "val_metrics": val_row, "test_metrics": test_row, "val_roc": val_roc, "val_pr": val_pr, "test_roc": test_roc, "test_pr": test_pr}


def route_dataset_to_matrix(ds):
    parts = []
    if ds.Xsn.numel() > 0:
        parts.append(ds.Xsn.cpu().numpy().astype(np.float32))
    if ds.Xsc.numel() > 0:
        parts.append(ds.Xsc.cpu().numpy().astype(np.float32))
    if ds.Xpn.numel() > 0:
        parts.append(ds.Xpn.cpu().numpy().astype(np.float32))
    if ds.Xpc.numel() > 0:
        parts.append(ds.Xpc.cpu().numpy().astype(np.float32))
    if len(parts) == 0:
        X = np.zeros((len(ds.y), 1), dtype=np.float32)
    else:
        X = np.concatenate(parts, axis=1).astype(np.float32)
    y = ds.y.cpu().numpy().astype(np.int64)
    return X, y

def align_proba(prob, classes_seen, n_classes):
    classes_seen = np.asarray(classes_seen, dtype=np.int64)
    full = np.zeros((prob.shape[0], n_classes), dtype=np.float32)
    for src_idx, cls in enumerate(classes_seen):
        if 0 <= int(cls) < n_classes:
            full[:, int(cls)] = prob[:, src_idx]
    row_sums = full.sum(axis=1, keepdims=True)
    zero_mask = row_sums.squeeze(1) <= 1e-12
    if np.any(zero_mask):
        full[zero_mask] = 1.0 / float(max(n_classes, 1))
        row_sums = full.sum(axis=1, keepdims=True)
    full = full / np.clip(row_sums, 1e-12, None)
    return full.astype(np.float32)

def make_classical_model(model_name, n_classes):
    if model_name == 'SVM':
        clf = SVC(C=1.0, kernel='rbf', probability=True, class_weight='balanced', random_state=SEED)
        return Pipeline([('imp', SimpleImputer(strategy='median')), ('scaler', StandardScaler()), ('clf', clf)])
    if model_name == 'Logistic Regression':
        clf = LogisticRegression(C=1.0, max_iter=500, class_weight='balanced', random_state=SEED)
        return Pipeline([('imp', SimpleImputer(strategy='median')), ('scaler', StandardScaler()), ('clf', clf)])
    if model_name == 'GaussianNB':
        clf = GaussianNB()
        return Pipeline([('imp', SimpleImputer(strategy='median')), ('clf', clf)])
    if model_name == 'AdaBoost':
        clf = AdaBoostClassifier(n_estimators=120, learning_rate=0.8, random_state=SEED)
        return Pipeline([('imp', SimpleImputer(strategy='median')), ('clf', clf)])
    if model_name == 'LDA':
...
def decision_to_proba(scores, n_classes):
    ...
    exp_scores = np.exp(np.clip(scores, -40.0, 40.0))
    return (exp_scores / np.clip(exp_scores.sum(axis=1, keepdims=True), 1e-12, None)).astype(np.float32)

def fit_predict_classical_model(model_name, Xtr, ytr, Xva, Xte, n_classes):
    model = make_classical_model(model_name, n_classes)
    model.fit(Xtr, ytr)
    classes_seen = None
    if hasattr(model, 'classes_'):
        ...
        for step in reversed(list(model.named_steps.values())):
            if hasattr(step, 'classes_'):
                classes_seen = step.classes_
                break
    if classes_seen is None:
        classes_seen = np.arange(n_classes)
    if hasattr(model, 'predict_proba'):
        pva = model.predict_proba(Xva)
        pte = model.predict_proba(Xte)
    else:
        pva = decision_to_proba(model.decision_function(Xva), n_classes)
        pte = decision_to_proba(model.decision_function(Xte), n_classes)
    pva = align_proba(np.asarray(pva, dtype=np.float32), classes_seen, n_classes)
    pte = align_proba(np.asarray(pte, dtype=np.float32), classes_seen, n_classes)
    return pva, pte

def run_classical_ml_variant(model_name, specs):
    set_seed(SEED)
    all_meta, all_tr, all_va, all_te, all_cidx, _ = prepare_standard_fl_bundle(specs)
    val_preds = {}
    test_preds = {}
    val_metrics_each = []
    test_metrics_each = []
    for meta, tr_ds, va_ds, te_ds in zip(all_meta, all_tr, all_va, all_te):
        Xtr, ytr = route_dataset_to_matrix(tr_ds)
        Xva, yva = route_dataset_to_matrix(va_ds)
        Xte, yte = route_dataset_to_matrix(te_ds)
        pva, pte = fit_predict_classical_model(model_name, Xtr, ytr, Xva, Xte, meta['n_classes'])
        val_preds[meta['name']] = {'y': yva, 'p': pva}
        test_preds[meta['name']] = {'y': yte, 'p': pte}
        val_metrics_each.append(compute_metrics(yva, pva))
        test_metrics_each.append(compute_metrics(yte, pte))
    val_row = evaluate_global_weighted(val_metrics_each, val_preds, all_meta, 'VAL')
    test_row = evaluate_global_weighted(test_metrics_each, test_preds, all_meta, 'TEST')
    val_row['variant'] = model_name
    test_row['variant'] = model_name
    yv_bin_all = []
    pv_bin_all = []
    yt_bin_all = []
    pt_bin_all = []
    ...
    if yv_bin_all:
        yv_bin = np.concatenate(yv_bin_all)
        pv_bin = np.concatenate(pv_bin_all, axis=0)
        val_roc = collect_roc_data(yv_bin, pv_bin, f'{model_name}_val_binary', ['Normal', 'Attack'])
        val_pr = collect_pr_data(yv_bin, pv_bin, f'{model_name}_val_binary', positive_label='Attack')
    if yt_bin_all:
        yt_bin = np.concatenate(yt_bin_all)
        pt_bin = np.concatenate(pt_bin_all, axis=0)
        test_roc = collect_roc_data(yt_bin, pt_bin, f'{model_name}_test_binary', ['Normal', 'Attack'])
        test_pr = collect_pr_data(yt_bin, pt_bin, f'{model_name}_test_binary', positive_label='Attack')
    gc.collect()
    return {'variant': model_name, 'val_metrics': val_row, 'test_metrics': test_row, 'val_roc': val_roc, 'val_pr': val_pr, 'test_roc': test_roc, 'test_pr': test_pr}

classical_models = ['SVM', 'Logistic Regression', 'GaussianNB', 'AdaBoost', 'LDA', 'Ridge Classifier']
specs = download_dataset_specs([dict(x) for x in DATASET_SPECS])
results = [run_classical_ml_variant(name, specs) for name in classical_models]
...
with open(os.path.join(OUTPUT_DIR, 'grip_dffi_classical_machine_learning_validation_6models_fast_metrics_and_curves.json'), 'w') as f:
...
<div class="grip-table-block"><div class="grip-table-title">GLOBAL VALIDATION METRICS</div><table class="dataframe">
  <thead>
...
<div class="grip-table-block"><div class="grip-table-title">GLOBAL TEST METRICS</div><table class="dataframe">
  <thead>
    <tr style="text-align: right;">
      <th>variant</th>
      <th>acc</th>
...
![png](12-classical-machine-learning-validation%20%281%29_OUTPUT_VIEW_files/12-classical-machine-learning-validation%20%281%29_OUTPUT_VIEW_0_3.png)
    



    
![png](12-classical-machine-learning-validation%20%281%29_OUTPUT_VIEW_files/12-classical-machine-learning-validation%20%281%29_OUTPUT_VIEW_0_4.png)
    



    
![png](12-classical-machine-learning-validation%20%281%29_OUTPUT_VIEW_files/12-classical-machine-learning-validation%20%281%29_OUTPUT_VIEW_0_5.png)
    



    
![png](12-classical-machine-learning-validation%20%281%29_OUTPUT_VIEW_files/12-classical-machine-learning-validation%20%281%29_OUTPUT_VIEW_0_6.png)
