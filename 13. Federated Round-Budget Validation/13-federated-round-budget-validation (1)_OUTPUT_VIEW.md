...
LR = 1e-3
BATCH_SIZE = 256
OUTPUT_DIR = "grip_dffi_federated_round_budget_validation_outputs"
CURRENT_CFG = {}
...
def run_variant(variant_name, cfg, specs):
    ...
    for rnd in range(1, round_budget + 1):
        ...
        if global_val_acc > best_bundle["global_val_acc"]:
            ...
            stale_rounds += 1
        if early_stop and rnd >= min_rounds and stale_rounds >= patience:
            break
...
federated_round_budget_variants = [
    ("1 Round", {"round_budget": 1}),
    ("2 Rounds", {"round_budget": 2}),
    ("5 Rounds", {"round_budget": 5}),
    ("10 Rounds", {"round_budget": 10}),
    ("Early-Stop Selected Round", {"round_budget": 10, "early_stop": True, "early_stop_patience": 2, "early_stop_min_rounds": 2}),
]

specs = download_dataset_specs([dict(x) for x in DATASET_SPECS])
results = [run_variant(name, cfg, specs) for name, cfg in federated_round_budget_variants]
...
with open(os.path.join(OUTPUT_DIR, "grip_dffi_federated_round_budget_validation_metrics_and_curves.json"), "w") as f:
...
<table border="1" class="dataframe">
  ...
  <tbody>
    ...
    <tr>
      <th>4</th>
      <td>Early-Stop Selected Round</td>
      <td>0.9833</td>
      <td>0.9796</td>
      <td>0.9518</td>
      <td>0.9628</td>
...
![png](13-federated-round-budget-validation%20%281%29_OUTPUT_VIEW_files/13-federated-round-budget-validation%20%281%29_OUTPUT_VIEW_0_4.png)
    



    
![png](13-federated-round-budget-validation%20%281%29_OUTPUT_VIEW_files/13-federated-round-budget-validation%20%281%29_OUTPUT_VIEW_0_5.png)
    



    
![png](13-federated-round-budget-validation%20%281%29_OUTPUT_VIEW_files/13-federated-round-budget-validation%20%281%29_OUTPUT_VIEW_0_6.png)
    



    
![png](13-federated-round-budget-validation%20%281%29_OUTPUT_VIEW_files/13-federated-round-budget-validation%20%281%29_OUTPUT_VIEW_0_7.png)
