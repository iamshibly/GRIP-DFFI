...
def run_fedbn_dense(name, view, n_global_ids):
    ...
    for _ in range(2):
        ...
        for i, v in enumerate(view):
            m = models[i]
            base = state_to_cpu(m)
            merged = merge_states(base, {k: v for k, v in global_state.items() if not ('running_mean' in k or 'running_var' in k or 'num_batches_tracked' in k)})
...
class ShallowTabularEncoderHead(nn.Module):
    def __init__(self, in_dim, hid=128, n_classes=2):
        super().__init__()
...
class ScalarTransformerBackboneHead(nn.Module):
    def __init__(self, in_dim, d_model=96, n_heads=4, n_layers=2, ff_dim=192, n_classes=2):
        super().__init__()
        self.in_dim = in_dim
        self.scalar_w = nn.Parameter(torch.randn(in_dim, d_model) * 0.02)
        self.scalar_b = nn.Parameter(torch.zeros(in_dim, d_model))
        self.feature_emb = nn.Parameter(torch.randn(in_dim, d_model) * 0.02)
...
def init_general_models_for_view(view, model_factory):
    torch.manual_seed(SEED)
    np.random.seed(SEED)
    base_model = model_factory().to(DEVICE)
    base_state = state_to_cpu(base_model)
    models = []
    for _ in view:
        m = model_factory().to(DEVICE)
        load_state_safe(m, base_state)
        models.append(m)
    return models

def collect_shared_state(model, shared_key_rule):
    sd = state_to_cpu(model)
    return {k: v for k, v in sd.items() if shared_key_rule(k)}

def has_any_shared_keys(model, shared_key_rule):
    return any(shared_key_rule(k) for k in model.state_dict().keys())

def run_architecture_validation(name, view, model_factory, shared_key_rule):
    models = init_general_models_for_view(view, model_factory)
    use_shared = has_any_shared_keys(models[0], shared_key_rule)
    global_shared = collect_shared_state(models[0], shared_key_rule) if use_shared else {}
    for _ in range(2):
        local_shared_states, weights = [], []
        for i, v in enumerate(view):
            m = models[i]
            if use_shared:
                base = state_to_cpu(m)
                merged = merge_states(base, global_shared)
                load_state_safe(m, merged)
            for Xc, yc in v['clients']:
                train_dense_local(m, Xc, yc, epochs=1, lr=LR, train_backbone=True, train_head=True)
            if use_shared:
                local_shared_states.append(collect_shared_state(m, shared_key_rule))
                weights.append(sum(len(yc) for _, yc in v['clients']))
        if use_shared:
            weights = np.array(weights, np.float32)
            weights = weights / weights.sum()
            global_shared = average_dense_states(local_shared_states, weights)
    if use_shared:
        for m in models:
            base = state_to_cpu(m)
            merged = merge_states(base, global_shared)
...
def shared_except_head_rule(k):
    return not k.startswith('head.')

def shared_all_rule(k):
    return True

def shared_none_rule(k):
    return False
...
results = []
results.append(run_architecture_validation('Plain_MLP_Backbone', dense_view, lambda: DenseBackboneHead(in_dim=n_global_flat_ids, use_bn=False), shared_except_head_rule))
results.append(run_architecture_validation('Plain_Transformer_Backbone', dense_view, lambda: ScalarTransformerBackboneHead(in_dim=n_global_flat_ids), shared_except_head_rule))
results.append(run_architecture_validation('Shallow_Tabular_Encoder', dense_view, lambda: ShallowTabularEncoderHead(in_dim=n_global_flat_ids), shared_except_head_rule))
results.append(run_architecture_validation('Fully_Shared_Model', dense_view, lambda: DenseBackboneHead(in_dim=n_global_flat_ids, use_bn=False), shared_all_rule))
results.append(run_architecture_validation('Fully_Private_Model', dense_view, lambda: DenseBackboneHead(in_dim=n_global_flat_ids, use_bn=False), shared_none_rule))
...
show_table(
    'BACKBONE AND CORE ARCHITECTURE VALIDATION — GLOBAL METRICS',
    report,
    index=False,
    columns=cols,
...
plot_overlay(results, 'val', 'roc', os.path.join(ART_DIR, 'plots', 'backbone_architecture_val_roc.png'), 'Backbone and Core Architecture Validation — Validation ROC-AUC')
plot_overlay(results, 'test', 'roc', os.path.join(ART_DIR, 'plots', 'backbone_architecture_test_roc.png'), 'Backbone and Core Architecture Validation — Test ROC-AUC')
plot_overlay(results, 'val', 'pr', os.path.join(ART_DIR, 'plots', 'backbone_architecture_val_pr.png'), 'Backbone and Core Architecture Validation — Validation PR-AUC')
plot_overlay(results, 'test', 'pr', os.path.join(ART_DIR, 'plots', 'backbone_architecture_test_pr.png'), 'Backbone and Core Architecture Validation — Test PR-AUC')
...
with open(os.path.join(ART_DIR, 'backbone_architecture_validation_metrics.json'), 'w') as f:
    json.dump(metrics_json, f, indent=2)
with open(os.path.join(ART_DIR, 'backbone_architecture_validation_roc_pr_curves.json'), 'w') as f:
    json.dump(curves_json, f, indent=2)

print(os.path.join(ART_DIR, 'backbone_architecture_validation_metrics.json'))
print(os.path.join(ART_DIR, 'backbone_architecture_validation_roc_pr_curves.json'))
```
...
<div class="grip-table-block"><div class="grip-table-title">BACKBONE AND CORE ARCHITECTURE VALIDATION — GLOBAL METRICS</div><table class="dataframe">
  <thead>
  ...
  <tbody>
    ...
    <tr>
      <td>VAL</td>
      <td>Plain_Transformer_Backbone</td>
      <td>0.8579</td>
      <td>0.6087</td>
      <td>0.6798</td>
      <td>0.6351</td>
...
![png](9-backbone-and-core-architecture-validation%20%281%29_OUTPUT_VIEW_files/9-backbone-and-core-architecture-validation%20%281%29_OUTPUT_VIEW_0_2.png)
    



    
![png](9-backbone-and-core-architecture-validation%20%281%29_OUTPUT_VIEW_files/9-backbone-and-core-architecture-validation%20%281%29_OUTPUT_VIEW_0_3.png)
    



    
![png](9-backbone-and-core-architecture-validation%20%281%29_OUTPUT_VIEW_files/9-backbone-and-core-architecture-validation%20%281%29_OUTPUT_VIEW_0_4.png)
    



    
![png](9-backbone-and-core-architecture-validation%20%281%29_OUTPUT_VIEW_files/9-backbone-and-core-architecture-validation%20%281%29_OUTPUT_VIEW_0_5.png)
    


    artifacts_baseline_validation_kaggle_colab/backbone_architecture_validation_metrics.json
    artifacts_baseline_validation_kaggle_colab/backbone_architecture_validation_roc_pr_curves.json
